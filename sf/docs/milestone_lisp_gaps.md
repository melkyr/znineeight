...
| G31 | ComptimeEval + Emitter | `comptime_eval.zig`, `main.zig:377`, `c89_emit.zig` int_const emit | **FIXED (typed-comptime-values plan):** negative-literal / signed comptime `@intCast` folds previously aborted zig1 at compile (`PANIC: integer overflow in @intCast`, rc=134) because the evaluator was untyped modular-u64 (negate `0-v` wrapped, `@intCast` fold discarded the target, a *checked* u32 store panicked at `main.zig:377`). Fix = genuinely typed comptime values: `ComptimeVal { bits: u64, width_bits: u8, sig: bool }` threaded through `comptimeEvalEvaluate`/`BinOp`/`Builtin` (`sig` not `signed` — C89 keyword); untyped literals are signed; signed div/mod via pure-u64 sign/magnitude (zig0 lacks `@bitCast`/`@divTrunc`/`@rem`); `@intCast` fold sets width/signedness and masks/sign-extends; new `U32ToU64Map` stores the full u64 bit pattern at `main.zig:377` (no checked narrow → no abort). Emitter renders signed negatives as the oracle form `(C_type)-magnitude` (width-masked two's-complement decode), unsigned/non-negative unchanged (byte-identical). Repros GREEN: `negative_intcast_ice` (−1), `comptime_signed_negate` (−1), `comptime_signed_divmod` (−3, `−6/2`), `comptime_i64_negative` (−1). Composed proof: original `@intCast(i32,-1)` `.Nil`-prong tagged-union form compiles and prints `142`. man/gol/mud byte-identical vs pre-plan `39b8c14b`; corpus `117/14/1/0`. Commits `f178b8a6` (typed evaluator + non-aborting store) + `9ad0ee3d` (signed emission). | Medium | ✅ Fixed |
