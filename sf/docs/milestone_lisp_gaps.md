...
| G30 | Lowerer + Emitter | `lower.zig` struct_init (`~2607`) + `c89_emit.zig` `emitFieldAssign`/`store_field` | **FIXED (tagged-union-payload-store Tasks 1-3):** tagged-union struct-init previously wrote only the `.tag`, dropping the payload (`Value{.Int=n}` → garbage). Lowerer now emits a void-guarded payload `assign_field{ TU_FIELD_PAYLOAD, src }`; emitter resolves `.payload.<Variant>._<sub>` on BOTH write branches by SRC-temp type (mirrors read path). Void variants emit no store (gol byte-identical). Also anon-`.{}` init type inferred via `expected_type_stack` (Task 4) so mud `.Go` returns store correctly. Repros GREEN: `tagged_union_payload` (payload stored; now reaches true end-state 142 after the T29 switch-capture-scoping fix, commit `e1c866fb`), `tagged_union_anon_return`, `anon_init_var_decl`/`if_arm`. | High | ✅ |
| G31 | ComptimeEval + Emitter | `comptime_eval.zig`, `main.zig:377`, `c89_emit.zig` int_const emit | **FIXED (typed-comptime-values plan):** negative-literal / signed comptime `@intCast` folds previously aborted zig1 at compile (`PANIC: integer overflow in @intCast`, rc=134). Fix = genuinely typed comptime values: `ComptimeVal { bits: u64, width_bits: u8, sig: bool }`; signed div/mod via pure-u64 sign/magnitude (zig0 lacks `@bitCast`/`@divTrunc`/`@rem`); new `U32ToU64Map` stores full u64 bits (no checked narrow); emitter renders signed negatives as `(C_type)-magnitude` (two's-complement decode); unsigned/non-negative unchanged (byte-identical). New `itoa64` renders u64 magnitudes so i64 folds > 2^32 (e.g. `@intCast(i64,-5000000000)`) do not panic at the emitter. Repros GREEN: `negative_intcast_ice` (−1), `comptime_signed_negate` (−1), `comptime_signed_divmod` (−3), `comptime_i64_negative` (−1), `comptime_i64_big_negative` (1). Composed proof: `@intCast(i32,-1)` `.Nil`-prong tagged-union prints `142`. man/gol/mud byte-identical vs `39b8c14b`; corpus `117/14/1/0`. Commits `f178b8a6` (typed evaluator+store) + `9ad0ee3d` (signed emission) + `dabcd11c` (itoa64). | Medium | ✅ Fixed |
...
| T29 | Switch capture-name-reuse READ bug — **FIXED (switch-capture-name-reuse plan, commit `e1c866fb`).** Two sequential switches reusing capture `\|v\|` over same-typed payload variant previously bound the 2nd switch's capture read to the 1st switch's value (predictable `actual = N*A`, later payloads dropped). Root cause: switch prong-set reset cleared `capture_shadow` but never popped `local_decl_*`, so the first switch's capture leaked; first-match binding + c89 decl dedup cemented the collision. Fix = Approach B per-switch capture SCOPING: save `saved_ldc = local_decl_count` at each switch prong-set start and restore it at each switch reset site (alongside `capture_shadow.count=0`), so each switch's captures resolve to their own payload temp. `maybeDisambiguateCapture` left intact. Repro `repro/switch_capture_name_reuse` RED 20 → GREEN 15; `repro/tagged_union_payload` reaches its true end-state 84 → **142** (payload store G30 + capture scoping together). | `lower.zig` switch save/restore (`~2754`/`~2824` expr, `~3407`/`~3443` stmt) | ✅ Fixed |
| T30 | Tagged-union slice-payload write regression — **FIXED (tagged-union-slice-payload plan, commit `0284d7a4`).** `Token{ .Symbol = sym }` emitted bare `.payload = sym` in C89, breaking gcc compilation of `examples/z98/lisp_interpreter_curr`. Fault A (root, latent): `semantic_analyzer.zig:1602` `semanticAnalyzerResolveSliceExpr` hardcoded `is_const=false`, typing `self.input[a..b]` (base `[]const u8`) as `[]u8`; slice canonicalization keys on const so `[]u8` ≠ `[]const u8` type_ids; emitter variant match by exact type_id failed. Fault B (regression trigger): commit `cce4b70f` (tagged-union Task 10) removed the `lower.zig` retype patch that had masked Fault A. Fix = correct-layer const propagation at `semantic_analyzer.zig:1602` (propagate `is_const` from base slice flags); Option L (backend-agnostic LIR) not needed. Design intent documented in `docs/sf/TYPE_SYSTEM_p2.md §7.5`. Result: `lisp_interpreter_curr` compiles clean (0 gcc errors, 0 bare payload writes); runs `42`→42, `nil`→nil. **SEPARATE:** `(+ 1 2)`→`Eval error: Other` — pre-existing lisp eval/apply bug, documented, not chased. Repro `repro/tagged_union_slice_payload`=5; corpus 117/14/1/0; man/gol/mud byte-identical. | `semantic_analyzer.zig:1602`, `lower.zig` (ex-mask cce4b70f) | ✅ Fixed |

| T31 | Optional-null var-decl init lowering + `== null` comparison broken — **FIXED (optnull-vardecl-init plan, commits `87703500`+`804f28a1`).** `var x: ?T = null;` miscompiled to a self-assign `x = x;` (lower.zig fast-path `:3579-3591` used `dl_temp` as both `set_optional_null` target and `assign` source → `p = p;` in emitted C; secondary: c89_emit `set_optional_null` rendered via `mangleTempName` so `has_value = 0` landed on `zT_N` temp not the local). `optional == null`/`!= null` comparison lowered as raw struct `==` int (gcc compile error). Fix: Approach B (repair fast-path in place: keep `set_optional_null` on local + drop self-assign; c89_emit `mangleTempName`→`resolveTempName` for set_optional_null emission) + pure-lowerer comparison fix (detect `null_literal` vs optional operand, emit `check_optional(→has_val)`+`UN_NOT` for `==`, return `has_val` for `!=`; reuses existing LIR at `lower.zig:1241`). Design intent documented in `docs/sf/AST_LIR_Lowering_p2.md §2.2+§3.1`, `LIR_C89_Emission_p2.md`, `Design_p2.md`. Result: `lisp_interpreter_curr` compiles clean (0 gcc errors, 0 self-assigns on `global_env`/`tail_env_head`), env chain constructible (atoms work). Repro `repro/optional_null_vardecl_init` prints `11`; man/gol/mud byte-identical; corpus 102/30/0/0 (no regression). **SEPARATE:** `(+ 1 2)`→`Eval error: Other` — pre-existing lisp eval/apply bug, out of scope, documented for future plan. | `lower.zig:3579-3591`+`:1241`, `c89_emit.zig:2956-2962` | ✅ Fixed |
| T32 | Eval error codes masked — `catch \|err\|` reuse lowered to same local, second catch error goes to unnamed temp — **FIXED (capture-rename-unification plan, commit `317f507e`).** Two `catch \|err\| { body }` blocks in `lisp_interpreter_curr/main.zig` emitted `decl_local("err", temp_A)` and `decl_local("err", temp_B)` with same name_id. `c89_emit.zig:1659` dedup excluded temp_B from `fl_temps`, so `resolveTempName(temp_B)` fell back to `mangleTempName` → emitted `zT_341` instead of `err`. Error switch read uninitialized `err` → all eval errors showed "Other". Fix: (1) extended `maybeDisambiguateCapture` to rename on ANY name conflict (not just type mismatch), (2) called it at catch-expr lowering `lower.zig:~2446` so second catch binding renamed to `err_1`, (3) removed save/restore `local_decl_count` scope patches from switch captures (redundant after rename unification). Single mechanism: `maybeDisambiguateCapture` now uniform at all 5 capture sites (switch-expr:2804, switch-stmt:3450, for-range:3292, for-in:3347, catch-expr:2446). Result: `(+ 1 2)` → `Eval error: UnboundSymbol` (real error code visible; error masking **fixed**). **SEPARATE:** builtins (`+`, `nil?`, etc.) not registered in env — `UnboundSymbol` is a real lookup failure, pre-existing separate bug, documented for future plan. | `lower.zig:494-520` (maybeDisambiguateCapture), `lower.zig:2446` (catch), `c89_emit.zig:1659` (dedup) | ✅ Fixed |
| T33 | For-in loop counter infinite loop + array for-in missing ptr/len — **FIXED (for-in-counter-array-fix plan, commits `3b92ed21`+`5759de04`).** Slice/range for-in computed `nxt = binary(add, idx, 1)` but only did local Zig `idx_temp = nxt_idx` — no LIR `assign` emitted → C output had `zT_n = zT_m + 1` but never `zT_m = zT_n` → infinite loop (caught by capture_rename Case 3). Array for-in called `load_field(.ptr/.len)` on array type → gcc error (arrays lack .ptr/.len). Fix: (1) emitter `LirInst.assign(dst=idx_temp, src=nxt_idx)` before local reassign at lower.zig:3318(stmt)+3357(slice) so loop counter stores back; (2) `array_type` branch in for-in handler uses array temp directly as C pointer + `int_const(array_items[].length)` for length, sharing cond/body/incr code with slice path. Result: capture_rename Case 3 prints 30 (was infinite loop), Case 6 prints 30 (was while workaround), array for-in green. Repro repro/for_in_counter=60 rc=0, repro/for_in_array=60 rc=0; man/gol/mud byte-identical. | `lower.zig:3318`+`:3357` (assign), `lower.zig:3326-3333` (array branch) | ✅ Fixed |

| T34 | First field store silently dropped — `node.symbol = name` never emitted in env_extend | All builtins registered with empty symbol → env_lookup returns UnboundSymbol | `lower.zig:findLocalTemp` used 0 as both valid temp (first param=temp 0) and "not found" sentinel; `plain_assign:1376` `if (src==0)` skipped store for aggregate-typed params (slice/struct) that bypass load_local at ident_expr:1673-1676 | ✅ Fixed | `c8b2951c` | (+ 1 2) → 3; all builtins found and work | | | `lower.zig:926` (findLocalTemp ?u32), `lower.zig:1373` (plain_assign guard) | ✅ Fixed |
| T35 | Lambda evaluation returns error 259 — `((lambda (x) x) 42)` fails | closures unusable, all user-defined functions broken | `try_expr` error-union lowering `lower.zig:2410` set `self.block_terminated=1` on the error/`ret` path but never reset it when switching to the success block `ok_bb` (`:2411`); stale flag suppressed the guarded `.jump=join_bb` (`:2414`) and the downstream stmt-switch prong-exit jump (`:3487`), leaving `ok_bb`/`join_bb` with no terminator → C89 emitter fell through in physical block-array order into the adjacent `too_few`-switch Nil-case block → premature `too_few` check with garbage `too_few=-1` → spurious `TooFewArgs` (259) during closure application. Fix = insert `self.block_terminated = @intCast(u8, 0);` after `:2411`, mirroring `catch_expr:2445`. | ✅ Fixed | `2f855b62` | `((lambda (x) x) 42)`→42, `(fact 5)`→120, `(fib 7)`→13, mutual `(even? 4)`→true; corpus 117/14/1/0; man/gol/mud byte-identical. **NOTE:** `define` returns the bound value (`<closure>`), not `nil` — byte-identical to zig0 oracle, so oracle-correct (plan's `nil` expectation was a spec error). | `lower.zig:2412` (block_terminated reset on try_expr success path) | ✅ Fixed |
...
|----|---------|-------------------|-------|-------|--------|
| EX1 | `error[48]` unsupported field-store base (ICE) — cross-module `@import` + named-const-sized array-of-struct `[N]struct{x:i32}` + array-element field store `t.arr[0].x=1` (each factor load-bearing; misleading name — also fires on array-element stores) | lzw, json_parser_workaround, lisp_interpreter | lowerer (`iceFieldStoreUnsupported` lower.zig:733/758) | `xmod_field_store_index` | ✅ GREEN (2026-07-13) |
| EX2 | `error[3002]` unhandled node kind in type resolution (ICE) — bare range-for `for (1..13) \|i\| {}` | days_in_month | type resolution | `typeres_unhandled_node` | ✅ Fixed (2026-07-14) |
