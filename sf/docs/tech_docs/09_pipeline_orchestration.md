...
- `astStoreHasPrintRef` — Task 1 (fix round 1): linear AST scan for ANY `ident_expr`/`field_access`
  ...
  On a hit, `moduleRegistryResolveImport("std_fmt.zig", root)` + a second
  `moduleRegistryResolveImports` bring the formatting module (and its PAL backing) into the graph;
  the user needs no new import. Over-approximation is safe (an unreferenced std_fmt is pruned).
  [updated: 2026-09-24 — Task 1 fix round 1]
