...
- `astStoreHasPrintCall` — Task 1: linear AST scan for a `fn_call` whose callee is an
  `ident_expr`/`field_access` named `print` (the same syntactic shape the lowerer's print special
  case intercepts). On a hit, `moduleRegistryResolveImport("std_fmt.zig", root)` + a second
  `moduleRegistryResolveImports` bring the formatting module (and its PAL backing) into the graph;
  the user needs no new import. [updated: 2026-09-24 — Task 1]
...
'<path>'`. Both are caught by the first `hasErrors` gate (after phase 3) → exit 2.
