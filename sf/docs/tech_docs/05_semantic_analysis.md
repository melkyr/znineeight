...
### semanticAnalyzerCheckLocalShadow (`semantic_analyzer.zig`)  [added: 2026-09-22 — Task 7D]
...
semanticAnalyzerCheckLocalShadow(name_id, span_start, span_end):
  ...
  scan local_decl_names[local_decl_count-1 .. 0] for name_id
    hit -> error[3057] "local declaration shadows an earlier declaration in an enclosing scope"
           at [span_start, span_end) + note "previous declaration here"; return
...
- **array-field `.len` fallback** `[updated: 2026-09-21 — Task 11N]`: before the final `TYPE_VOID` fallback, `semanticAnalyzerArrayFieldLen(node.child_0)` inspects the `.len` base. If it is a `field_access` whose container (struct / union / packed_union, optionally through a pointer) declares the named field as an `array_type`, `.len` resolves to `TYPE_USIZE`. This recovers `s.a.len` after the Phase-4 array-field decay (`s.a` → `*u8`) makes the `array_type` arm above unreachable. The fallback is gated on the accessed name being `len` (`field_name_id == len_id`); an unknown field on an array field (`s.a.foo`) now falls through to the Phase-5 `error[3060]` reject (Task 13), not a silent `TYPE_VOID`. A `[*]T` field (many-item pointer) is not an array and is now rejected cleanly with `error[3000]` by the Task 11S (b) many-ptr `.len` gate (previously a silent `TYPE_VOID`).
...
**Phase 5 — not found:** marker `NF\n FF2:N FF2:F FF2:B`. [updated: 2026-09-24 — Task 13 (S1)]: a member not found on a `struct_type`/`union_type`/`packed_union_type`/`tagged_union_type` base now emits the dedicated level-0 `error[3060]` (`ERR_3060_METHOD_SYNTAX_NOT_SUPPORTED`) with message `no field or member function named '<name>' in struct/union type`, then returns `TYPE_VOID` (rc=2, 0 `.c`). Z98 aggregate types cannot contain function declarations, so the spec-forbidden method syntax `value.func()` is always this shape (matching official Zig 0.15.2's "no field or member function named '...'"); previously the access resolved silently to `TYPE_VOID` and a call lowered to an undeclared callee temp (`zT_N = zT_undeclared();`, gcc/link failure with no diagnostic). Deduped per node via `diagnosticCollectorMarkNodeOnce` (the expression walk can revisit a field access). Enum/error-set members and non-aggregate bases keep the pre-existing silent `TYPE_VOID` fallbacks.
