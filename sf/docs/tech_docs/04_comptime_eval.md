...
type); `checkFoldedIntFits` covers `materializeInto` (after optional/error-union layer unwrap),
`applyCoercion`, the module/local decl slots and `return_stmt`; the builtin HIT range-checks a
concrete integer target. Failures emit the preserved `error[3000]` (new message
`"comptime integer value does not fit the target type"`; the `@intCast`/`@as` fold keeps its own
messages) at most once per node, and the post-lowering diagnostic gate exits rc=2 with 0 `.c`.
`~` folds stay exempt from the materialisation fit (the Task 3 documented divergence).
...
backing + the carry-item i64 min; stdlib pin 220 → 221),
`repro/mi_matrix/comptime_coerce_reject_xmod` (7 lowering-phase `error[3000]` sites),
`repro/mi_matrix/array_size_negative_reject_xmod` (`error[3050]`), and the standalone
`repro/comptime_coerce_typed_slots.z98` + `repro/comptime_coerce_reject.z98`.
...
9. **Comparison peer-fit / Zig-matching rejection cases (Task 3; the Task 9D bounded divergence is
   ...
   v207. Float comparison folding remains out of scope.

10. **Unannotated MODULE int consts keep their symbol slot type (Task 4 residual, pre-existing).**
    Task 4 re-types an unannotated LOCAL binding by exact value (risk 7), but a module-scope
    unannotated const's C type comes from the symbol registry: `const X = 2000000000 +
    1000000000;` still materialises the fold exactly (U32 temp) but stores it into the global's
    `int` slot and truncates at runtime (`u3` probe). The out-of-64-bit module case is rejected
    (the untyped HIT); only the in-window slot-type selection is unfixed. Local annotations or a
    module annotation (`const X: u32 = …`) are correct.

11. **Optional-payload coercion is not range-checked when no wrap is recorded (Task 4 scope
    boundary).** `takeOpt8(@as(i32, 300))` / `var o: ?u8 = @as(i32, 300);` are accepted with
    `warning[3000]` (Zig rejects `expected type '?u8', found 'i32'`): sema records no optional
    wrap for this shape, so no `materializeInto` call happens and the pinned folded-value check
    never sees the payload. The direct integer parameter/return/decl sites are checked.
