...
| Comparison ops evaluated | 6 | cmp_eq, cmp_ne, cmp_lt, cmp_le, cmp_gt, cmp_ge (Task 9D; **Task 3:** exact signedness-free magnitude+sign over the big-int operands; bools compare as 0/1; **Task 9:** a float operand dispatches to `comptimeEvalCompareFloat`, which folds at the established f64 precision under the exact-representability peer rule) |
...
| `comptimeEvalBuiltin` — `@sizeOf` | — | Extract first extra child as type arg, resolve, return `ty.size` as `ComptimeVal`. | (same as above) | same | `registry.types_items[t].size` | Always `KIND_INT` (compile-time size is unsigned). | None [inference] |
| `comptimeEvalBuiltin` — `@alignOf` | — | Same pattern as `@sizeOf` but returns `ty.alignment`. | (same as above) | same | `registry.types_items[t].alignment` | Always `KIND_INT`. | None [inference] |
...
| `comptimeEvalFloat` | private | Float-valued sub-evaluator, deliberately SEPARATE from `comptimeEvalEvaluateDepth` so no float bit pattern can reach the integer binop/negate/bit_not/int_cast paths. Handles `float_literal` (`store.float_values`), `negate` (`-v`, sign-preserving for `-0.0`), `paren_expr`, nested `@floatCast`/`@intToFloat`, and const `ident_expr` chains (depth-16 guarded). Fix round 1: the `ident_expr` arm resolves the const's **declared** type and rounds an `f32` const through `f32` before returning, so `const S: f32 = 0.1` evaluates as `f32(0.1)`, not the raw `f64` literal. **Task 9 (Part II):** the ident arm consults the enclosing function's local-const scope FIRST (mirroring the integer evaluator; a local `const` shadows a module const), so `const f: f64 = 1.5; if (f >= 1.5) …` folds. | `comptimeEvalFloatBuiltin`; `comptimeEvalCompareOperand`; recursively itself | `comptimeEvalFloat`, `comptimeEvalFloatBuiltin`, `comptimeEvalResolveTypeArg`, `symbolRegistryQualifiedLookup`, `type_resolver.localConstScopeLookup` | `store.float_values`, `store.nodes`, `store.identifiers`, `symbol_reg`, `local_consts` | `node_idx==0` or `depth>=16` → null. A non-float node kind → null. | None [inference] |
...
| `comptimeEvalCompare` | private | **Task 9D; Task 3 rewrite; Task 9 float path.** Fold a comparison (`cmp_eq`/`cmp_ne`/`cmp_lt`/`cmp_le`/`cmp_gt`/`cmp_ge`) to a bool `ComptimeVal` (`kind = KIND_BOOL`). Two integer/bool operands use the exact signedness-free magnitude+sign order (`ciCmp`: equal signs → `ciMagCmp` negated when negative; differing signs → the negative one is smaller; `-0` is normalized to `0`, bools are 0/1); there is no sign-class/declared-type inference and no `ciValToOldBits` adapter (the Task 9D bounded divergence is retired). Comparisons are deliberately EXEMPT from the arithmetic peer-fit rule (the oracle accepts `const u: u8 = 200; u > -1` → true and `u > 300` → false). **Task 9 (Part II):** if either operand evaluates as a `KIND_FLOAT`, the call dispatches to `comptimeEvalCompareFloat` (below) instead of declining — comptime float comparisons now fold at the established f64 precision. | `comptimeEvalEvaluateDepth` | `comptimeEvalEvaluateDepth` (recursive for lhs/rhs), `ciCmp`, `ciBoolVal`, `comptimeEvalCompareFloat` | `store.nodes`, lhs/rhs `ComptimeVal` | Either operand unfoldable → null. No declared type, no sign class, no 64-bit window. | None [inference] |
| `comptimeEvalCompareFloat` | private | **Task 9 (Part II)** — the float comparison fold. Evaluates both operands via `comptimeEvalCompareOperand` (which tries the exact integer evaluator first — a `KIND_FLOAT` result carries its `float_bits`, an int carries its exact value plus `ciSignificantBits`, a bool is declined — then the `comptimeEvalFloat` sub-evaluator). Peer rules verified against Zig 0.15.2: a typed `f64` operand makes the peer f64 (an `f32` widens exactly); otherwise a typed `f32` peer folds an UNTYPED operand only when its f64 value is exactly f32-representable (Z98's emitted C widens to double, so rounding would fold a verdict the runtime never computes); with no typed float operand the comparison is `comptime_float`, evaluated at f64 (the documented precision residual). An integer operand participates only when `sig_bits <= 53` (f64 peer) or `<= 24` (f32 peer) so the value is exact in the peer; otherwise the fold declines (no verdict invented). The six operators compare the two f64 values (IEEE, `-0.0 == 0.0`). | `comptimeEvalCompare` | `comptimeEvalCompareOperand`, `comptimeEvalFloat`, `comptimeEvalF64IsF32Exact`, `ciBoolVal` | `store.nodes`, lhs/rhs `CmpFloatOperand` | A non-foldable operand, a bool-vs-float shape, or an inexact integer/`f32`-peer operand → null (preserves the pre-Task-9 verdict). | None [inference] |
| `comptimeEvalCompareOperand` | private | **Task 9 (Part II).** One float-path comparison operand: the exact evaluator's `KIND_FLOAT` → `float_bits` reinterpreted (`comptimeEvalFloatBits`), `KIND_BOOL` → `is_bool` (declined by the caller), `KIND_INT` → `sig_bits = ciSignificantBits` + `fval = ciToF64`; otherwise the `comptimeEvalFloat` sub-evaluator supplies the value and `comptimeEvalFloatOperandType` supplies `ftype` (`TYPE_F32`/`TYPE_F64`/0 = untyped). | `comptimeEvalCompareFloat` | `comptimeEvalEvaluateDepth`, `comptimeEvalFloat`, `comptimeEvalFloatBits`, `comptimeEvalFloatOperandType`, `ciSignificantBits`, `ciToF64`, `ciIsZero` | `store.nodes`, `CmpFloatOperand` | Returns `?CmpFloatOperand` with `ok=false` when nothing folds. | None [inference] |
| `ciSignificantBits` | private | **Task 9 (Part II).** Significant-bit count of a `ComptimeInt` magnitude (0 for zero): bit length minus trailing-zero count. A value with `<= 53` (f64) or `<= 24` (f32) significant bits is exactly representable in that IEEE format. | `comptimeEvalCompareOperand` | limb shifts | `ComptimeInt.mag/len` | Scans low limbs for trailing zeros; the 256-bit cap is far inside the f64/f32 exponent range. | None [inference] |
| `comptimeEvalF64IsF32Exact` | private | **Task 9 (Part II).** f64 → f32 → f64 round-trip equality: `true` when the value is exactly an f32 value (used for an untyped operand under an f32 peer). | `comptimeEvalCompareFloat` | `@floatCast` | local `f64`/`f32` | NaN would be `false`; the evaluator cannot produce NaN. | None [inference] |
| `comptimeEvalFloatOperandType` / `…Depth` | private | **Task 9 (Part II).** The declared float type of a comparison operand: `TYPE_F32`/`TYPE_F64`, or 0 for an untyped `comptime_float`/non-float shape. Mirrors `comptimeEvalOperandTypeDepth` for the float kinds: unwraps parens (depth-32 guard), `negate` propagates, an `ident_expr` consults the function-local const scope first then the module symbols (annotation type, else recurse into an unannotated init), and `@intToFloat`/`@floatCast` report the resolved target. | `comptimeEvalCompareOperand`, `comptimeEvalFloatOperandType` | `comptimeEvalResolveTypeArg`, `type_resolver.localConstScopeLookup`, `symbolRegistryQualifiedLookup` | `store.nodes`, `local_consts`, `symbol_reg` | Returns `u32` (0 = untyped), depth cap 16. | None [inference] |
| `comptimeEvalFloatBits` | private | **Task 9 (Part II).** Reinterpret a folded float `ComptimeVal`'s `float_bits` as an `f64` (pointer cast; Z98 `@bitCast` is integer-only). | `comptimeEvalCompareOperand` | `@ptrCast` | `ComptimeVal.float_bits` | Inverse of the `comptimeEvalBuiltin` float-fold transport. | None [inference] |
...
- **Null return** — any evaluation that returns `null` means the node is NOT stored in
  `ctx.comptime_folds`, so LIR lowering falls back to a runtime form instead of an `int_const`.
  Null causes include: type argument unresolved (`ty.state != 2`), division/mod by zero, a negative or
  `> 2^32` shift count (a `shl` needing more than 256 bits also declines), unhandled node kind, `node_idx == 0`, a const chain deeper than 16, and any builtin
...
LIR lowering, where the `comptime_folds` table lookup turns a folded builtin into an `int_const`
...
and stores the result in `ctx.comptime_folds` via `comptimeFoldTablePut`. The lowerer's binary/unary
`comptime_folds` guards consume those folds as `int_const` LIR. Since F8 the evaluator itself
...
- Fold results are stored keyed by AST node index in `ctx.comptime_folds` (a `ComptimeFoldTable`) and consumed by the
  lowerer's `comptime_folds` lookups; a `@sizeOf`/`@alignOf`/`@offsetOf`/`@bitSizeOf`/`@bitOffsetOf`
...
### Float comparison folds (Task 9, Part II) — [updated: 2026-09-24]

**Defect.** `comptimeEvalCompare` returned `null` whenever an operand was float-valued, so a
comptime-true float condition never folded and a no-`else` value `if` was rejected
(`error[3059]`), while official Zig 0.15.2 folds it (`var x: i32 = if (0.5 < 1.0) 1;`).

**Fold.** `comptimeEvalCompare` keeps the exact integer `ciCmp` path when both operands are
integer/bool and otherwise calls the new private `comptimeEvalCompareFloat`. It evaluates each
operand through `comptimeEvalCompareOperand`: the exact evaluator first (a `KIND_FLOAT` result —
e.g. `@intToFloat`/`@floatCast` — contributes its `float_bits`, an int its exact value plus
`ciSignificantBits`, a bool declines), then the `comptimeEvalFloat` sub-evaluator (literals,
`negate`, parens, `@floatCast`/`@intToFloat`, and const chains including the function-local
scope — the Task 9 ident-arm addition). Peer rules, all verified against Zig 0.15.2:

- a typed `f64` operand makes the peer f64 (an `f32` operand widens exactly; an untyped literal
  is the f64 Zig coerces to);
- otherwise a typed `f32` operand makes the peer f32, and an UNTYPED operand folds only when its
  f64 value is exactly f32-representable (`comptimeEvalF64IsF32Exact`). It is NOT rounded: Z98's
  emitted C widens to double, so rounding would fold a verdict the runtime never computes;
- with no typed float operand the comparison is `comptime_float`, evaluated at f64 (the
  established precision; see KI 6);
- an INTEGER operand participates only when its magnitude's significant-bit count fits the peer's
  significand (`<= 53` for f64/`comptime_float`, `<= 24` for f32; new `ciSignificantBits`), so
  the integer→float coercion is exact and the verdict is the mathematical comparison Zig folds;
  otherwise the fold declines (never invents a verdict). A bool-vs-float shape declines.

The six operators compare the two f64 values with IEEE semantics (`-0.0 == 0.0`; the evaluator
cannot produce NaN). A folded bool condition feeds the Task 3 condition store, so lowering elides
the untaken branch exactly as for integer conditions.

**Fixtures.** `repro/mi_matrix/stdlib_comptime_float_compare_xmod` (`main.zig` + `expected.txt`/
`expected.rc`; 23 values: literal/literal all six operators, exponent and `-0.0` shapes, untyped
int vs float and an arithmetic-derived int, module + function-local f64 consts, typed f32
(f32-vs-f32, f32-vs-f32-exact literal, f32-vs-int), `@intToFloat(f64, …)` and
`@floatCast(f64, <f32>)`, logical combinations, an integer-comparison control, plus four
runtime-`if` false-condition controls; every value `@panic`-guarded and Zig-0.15.2-oracle-matched
3x) + standalone `repro/comptime_float_compare.z98`. RED pre-fix: `error[3059]` at the first float
condition. Bounded residuals (documented in KI 6/9): an `f32` operand against a non-f32-exact
untyped literal declines (Zig folds it, Z98's runtime comparison uses the double promotion), an
integer with more than the peer's significand bits declines (Zig compares exactly), and
`comptime_float` literals fold at f64 rather than Zig's f128.

**Fix round 1 (Critical + Important, 2026-09-24).** The original `ciSignificantBits` counted only
the TOP limb's bits and then subtracted 32 per zero lower limb, so it undercounted a multi-limb
magnitude (`2^64 + 1` → 1, `2^53 + 1` → 22, `2^32 + 1` → 1) and u32-underflowed on a power of two
≥ 2^32. That broke the fold contract in both directions: `if (((1 << 64) + 1) ==
18446744073709551616.0) 1;` was ACCEPTED (the rounded f64 compared equal, where Zig's exact compare
is false and pre-Task-9 Z98 rejected `error[3059]`), `const TU: u64 = 9007199254740993; if (TU >
9007199254740992.0) 1;` folded FALSE and materialised an uninitialized temp where Zig yields `1`,
and exactly-representable powers of two (`(1 << 32) > 1.0`, `1 << 63`, `1 << 64`, `1 << 255`)
false-declined. The helper now computes the true `bitlen(magnitude) - trailing_zeros`
(`32 * (len - 1)` plus the top limb's bit count, minus the separately accumulated trailing zeros).
Coverage: the positive fixture gains six multi-limb rows (30 values, golden 4 lines) and the new
reject fixture `repro/mi_matrix/comptime_float_compare_reject_xmod` pins the three Zig-false
over-acceptance sites at rc 2 / 3 × `error[3059]` / 0 `.c`. Fixed point `a059fa89…` →
`e809cf6113088dcd20ea18aeef5ea9e6`.
...
**Array sizes / enum initializers (Task 5).** The type-resolver evaluators are re-pointed at ONE
exact fold (`type_resolver.evalConstIntFull`, shared with this module's `ci*` core through the
deliberate mutual import): `evalConstU32Full` folds the size expression exactly
(`0..0xFFFFFFFE`; `0xFFFFFFFF` is the unfoldable sentinel) and rejects negatives/over-u32 results,
so `[0 - 1]u8` is `error[3050]` (Task 1 §9.3), while shifts/bitwise/parens and >64-bit
intermediates reduced back into range now fold (`[1 << 4]u8`, `[(1 << 200) >> 190]u8`; see doc 03).
`evalConstI64Full` materialises the exact value through the `[i64 min, u64 max]` member-storage
window, so `enum(u64) { A = (1 << 200) >> 190 }` folds `1024` and a 64-bit-wrapping initializer
(`18446744073709551615 + 1`) is a clean `error[3055]` instead of a silently stored `0`. The 64-bit
syntactic classifier `intValueFitsType`/`evalConstSignClass` is DELETED; `comptimeIntFitsType`
decides exactly.
...
- The lowerer emits `float_const` (not `int_const`) for these two callees, resolving the target
  ...
  pattern in `ctx.comptime_folds` (a `ComptimeFoldTable`; the f64 bits ride in `float_bits`) and reinterpreted at the consumer via a
...
- Fold coverage: float literals, `negate` (negative float literals are `negate(float_literal)`;
  ...
  and const `ident_expr` chains (**Task 9:** the chain also resolves function-local consts).
  Float arithmetic (`X + 1.0`) is still NOT folded; float COMPARISONS now fold — see the Task 9
  section above.
...
- **Item 2 (range-check 64-bit targets).** `comptimeValFitsType` used to `return true` for every
  ...
  by the sema probe, the global `phase_ComptimeEvaluation` fold is unaffected. **[Task 2/3/5 note:**
  the `comptime_eval.zig` side of this classifier was deleted — Task 2 replaced the 64-bit
  fits-check with the exact `comptimeIntFitsType`, Task 3 deleted `comptimeEvalSignClass`
  entirely, and Task 5 deleted the `type_resolver.zig` mirror too (`evalConstSignClass`,
  `EvalSignClass` and `intValueFitsType`): both type-resolver evaluators now share the exact
  `evalConstIntFull` over the same `ComptimeInt` core, so a negative source is structurally
  distinguished from a large non-negative value without any syntactic classification.**]** A definitely-negative source with the top bit set is
  rejected for an unsigned target; a definitely-non-negative source with the top bit set is rejected
  for a signed target. Fixtures:
...
- **`comptime_eval.zig`** — `ComptimeEval.host_is_windows: bool`; the `@isWindows` arm returns
  `ciBoolVal(self.host_is_windows)` — a `ComptimeVal{ v, kind = KIND_BOOL, float_bits = 0 }` with `v` = 1/0.
...
- `lower.zig` `builtin_call` arm — when the `comptime_folds` lookup hits and the callee is
...
3. **Division / shift guards return `null`** (`comptime_eval.zig`): `div`/`mod_op` by zero and
   `shl`/`shr` with a negative or `> 2^32` count fold to `null` with no diagnostic (a `shl` whose
   exact result needs more than 256 magnitude bits also declines); the runtime path then
...
6. **Float arithmetic does not fold; float comparisons DO (Task 9, Part II)** (`comptime_eval.zig`):
   only `@floatCast`/`@intToFloat` with comptime-known operands fold as conversions (`X + 1.0`
   stays runtime — the `kind == KIND_FLOAT` gate keeps the bit patterns out of the integer binop
   path). **Task 9 (Part II)** added the float comparison fold at the established f64 precision
   (`comptimeEvalCompareFloat`); bounded residuals: an `f32` operand compared against a non-f32-exact
   untyped literal declines (Zig peer-rounds it to f32, Z98's runtime comparison promotes to
   double), an integer with more significant bits than the peer's significand declines (Zig
   compares exactly), and `comptime_float` literal comparisons evaluate at f64 rather than Zig's
   f128. **Task 9D** folds integer comparisons and the bool logical ops; **Task 3** made the
   integer comparison signedness-free and exact (see above). **Fix round 1 (2026-09-24):**
   `ciSignificantBits` was corrected to `bitlen(magnitude) − trailing_zeros` — the original
   counted only the top limb's bits, so multi-limb magnitudes were undercounted (over-acceptance)
   and exact powers of two ≥ 2^32 u32-underflowed (false declines); the exact-representability
   rule now behaves as described, pinned by the 30-value positive fixture and the new
...
9. **Comparison peer-fit / Zig-matching rejection cases (Task 3; the Task 9D bounded divergence is
   ...
   **Task 9 (Part II)** added the float comparison folds — the remaining float residuals are the
   f64 literal precision and the inexact-peer declines listed in KI 6, pinned by
   `repro/mi_matrix/stdlib_comptime_float_compare_xmod`.
...
11. **Optional-payload coercion — the folded/literal shapes are now clean rejects; the
    ...
    warning[3000]+truncate declaration path (Task 1 §5.3; valid C, Zig rejects). Related
    coverage boundaries (Task 4 review round): the DECL on-demand probe cannot see a
    FUNCTION-LOCAL const (KI12(b)), and the ARG mode covers the fold table + bare
    literals only (doc 07).

12. **Task 4 review-round residuals (documented; out of Task 5 scope).** (a) A module-scope
    ANNOTATED const with an out-of-range literal (`const W: i8 = 200;`) is accepted with NO
    diagnostic and truncates silently in direct/call/field uses; in the anonymous-struct-argument
    shape (`std.io.print("{}", .{W})`) the emitted C references the const's undeclared global
    (gcc `'zG_…_W' undeclared`) — official Zig rejects the declaration itself
    (`type 'i8' cannot represent integer value '200'`). The UNANNOTATED module
    case is fixed (KI10). (b) The DECL on-demand probe cannot see a function-local const
    (`var y: u32 = c - 1;` with a local `c`), a false negative documented for doc 04/07.
    (c) Source integer literals `>= 2^64` clamp to `u64` max at lex/parse time
    (`astStoreIntValue`), so such a literal is accepted where Zig rejects it; the 256-bit cap
    governs arithmetic results, not literal spellings. (d) `+%`/`-%` wrapping init shapes were
    not probed (more permissive only). (e) The pinned bounded divergence `const BIGFOLD =
    1 << 100;` rejects `error[3000]` where Zig accepts (Task 1 §8 risk 1;
