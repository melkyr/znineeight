...
| `comptimeEvalCompare` | private | **Task 9D.** Fold a comparison (`cmp_eq`/`cmp_ne`/`cmp_lt`/`cmp_le`/`cmp_gt`/`cmp_ge`) of two integer `ComptimeVal`s to a bool `ComptimeVal` (`bits = 0|1`, `width_bits = 1`, `sig = false`). Evaluates both operands; a `WIDTH_FLOAT` operand returns null (float comparisons stay unfolded — a bounded residual). `use_signed = l.sig or r.sig`; signed operands are compared as `@bitCast(i64, bits)` (sign-extended by the producers), unsigned as `u64`. | `comptimeEvalEvaluateDepth` | `comptimeEvalEvaluateDepth` (recursive for lhs/rhs) | `store.nodes`, lhs/rhs `ComptimeVal` | Mirrors `comptimeEvalBinOp`'s div/mod sign handling. Either operand unfoldable → null. | None [inference] |
| `comptimeEvalLogical` | private | **Task 9D.** Fold `bool_and`/`bool_or`/`bool_not` on comptime bools. Requires the lhs `width_bits == 1`. `bool_not` returns `bits ^ 1`. `bool_and`/`bool_or` **short-circuit**: a decisive lhs (`0` for `and`, `1` for `or`) returns immediately WITHOUT evaluating the rhs (so `true or <runtime>` folds true, `false and <runtime>` folds false); otherwise the rhs is evaluated and must also be bool-width. | `comptimeEvalEvaluateDepth` | `comptimeEvalEvaluateDepth` (recursive for lhs, and rhs when not short-circuited) | `store.nodes`, lhs/rhs `ComptimeVal` | A non-bool lhs/rhs, or an unfoldable rhs on a non-decisive lhs, returns null. | None [inference] |
...
comptimeEvalEvaluate(node_idx)
  └─ comptimeEvalEvaluateDepth(node_idx, depth=0)     ← every recursion carries depth
       ...
       │           └─ signed if l.sig | r.sig (i64 compare); else u64; WIDTH_FLOAT → null
       ├─ bool_and/bool_or ──→ comptimeEvalLogical(node_idx, kind, depth)
       │     └─ short-circuit: decisive lhs returns without evaluating the rhs
...
- **Item 2 (range-check 64-bit targets).** `comptimeValFitsType` used to `return true` for every
  ...
  so valid programs are not over-rejected). A definitely-negative source with the top bit set is
