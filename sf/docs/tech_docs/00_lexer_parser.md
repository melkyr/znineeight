...
| `parserParseIfExpr` | 782 | private (but referenced pub in parsePrimary) | `[inference: if(cond) [|capture|] then-expr [else else-expr]]` | If-expression (expression-context). **[F-PARSERGAP, 2026-08-14] Now supports optional-capture `|name|` in VALUE position** — leading-pipe capture block verbatim from `parserParseIfStmt` (parser.zig:787-797); `if_expr` node `payload` = capture node (parser.zig:810; was hardcoded `0`). No-pipe `if (x)` unchanged (payload stays `0` → byte-identical AST). Lowerer (lower.zig if_expr path) binds the capture via `bindOptionalCapture` (orig cond temp) and converts the optional cond (`check_optional` → `has_val`) before `.branch`; the comptime-fold sub-path is guarded `payload==0` (captures need an optional cond, which `comptime_values` never folds). |
