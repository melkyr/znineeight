...
| `parserParseForStmt` | private | `for (range) [\|elem, idx\|] body ;`; range `..` wrapped in `range_exclusive`; `child_2` = index name id. Task 11: `for (iterable, start..) [\|elem, idx\|]` / `for (iterable, start..end) [...]` wraps the iterable + index range in a `for_index_range` pattern node (child_1 = start, child_2 = end or 0). |
...
| Type | Description |
|------|-------------|
| `AstKind` (enum u8) | 113 variants (0..112). `mod_assign=74`, distinct from `swt_ex=56`; `for_index_range=112` (Task 11) is the explicit-index `for` pattern. |
