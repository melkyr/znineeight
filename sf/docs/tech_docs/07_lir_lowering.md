...
- `return_stmt`'s `hoisted_temps[val]` deref is guarded against `TEMP_NONE` so a void value-`if` in
  ...
  `return;` rather than `return zT_4294967295;`. A void `catch` block that lowers to a real temp
  (e.g. `_ = writeCoreDump(...) catch {};`) keeps its pre-existing store — a separate shape.
