...
#### Capture-shadow redirect (scope-chain guard) `[updated: 2026-09-24 — Task 10]`

`maybeDisambiguateCapture` renames a `for`/`while`/`catch` capture when the local-decl table already
holds a same-named declaration from a closed scope (even a sibling's), and records the source-name →
synthetic-name redirect in `capture_shadow` (`U32ToU32Map`). That map is **function-scoped**:
`main.zig` creates a fresh `LirLowerer` per `fn_decl`, and `lowerFn` additionally resets the
function-scoped bookkeeping at entry (`local_decl_count = 0` +
`hash_mod.u32ToU32MapClear(&self.capture_shadow)`) so the invariant also holds for callers that reuse
one lowerer across functions (the unit tests). `u32ToU32MapClear` (util/hash.zig) is a real clear —
it zeroes the `occupied` flags and `count`; the eight historical arm-end
`self.capture_shadow.count = 0` stores were dead (`u32ToU32MapGet` scans `occupied` and ignores
`count`) and were deleted. A mid-function clear remains wrong: it would drop an enclosing capture
that is referenced after a nested loop inside its own body.
...
(declaration + initializer dropped, uses aliased the old loop variable). `maybeDisambiguateCapture`
still renames conservatively whenever ANY earlier same-name declaration exists (including closed
sibling scopes); Task 10 kept that behavior deliberately — it is semantically neutral and narrowing it
would churn the emitted C of existing valid programs (e.g. `examples/z98/json_parser`'s
`item_1`/`i_2`).
