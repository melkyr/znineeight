...
**Comptime-values HIT (fold consumer).** The `builtin_call` arm checks `ctx.comptime_values[node_idx]` before any runtime dispatch. On a hit the temp type defaults to `TYPE_USIZE`, is overridden for `@intCast` (resolved target), `@isWindows` (`TYPE_BOOL`), and — Task 11D — `@floatCast`/`@intToFloat` (resolved `f32`/`f64` target), then emits `int_const` — except `@floatCast`/`@intToFloat`, whose map entry is an f64 bit pattern and which therefore emit `float_const` (the u64 is reinterpreted back to f64 via a pointer cast, since Z98 `@bitCast` is integer-only). A miss falls through to the runtime builtin forms below.
