...
| `capture_shadow` | `U32ToU32Map` | Maps captured names to disambiguated synthetic names. Append-only for the whole session: the eight arm-end `capture_shadow.count = 0` sites are dead writes (`u32ToU32MapGet` ignores `count`), so a redirect is decided by `captureShadowShouldRedirect`'s scope-chain walk (see Variables §Capture-shadow redirect) |
...
#### Capture-shadow redirect (scope-chain guard) `[updated: 2026-09-23 — Task 10D]`

`maybeDisambiguateCapture` renames a `for`/`while` capture when the append-only local-decl table
already holds a same-named declaration from a closed scope (even a sibling's), and records the
source-name → synthetic-name redirect in `capture_shadow` (`U32ToU32Map`). That map has **no real
scope exit**: the eight `self.capture_shadow.count = 0` sites at the loop-arm ends are dead writes
because `u32ToU32MapGet` scans the `occupied` flags and ignores `count`, so entries survive for the
whole compilation session. Every identifier read/store/l-value that names a shadowed capture
therefore calls `captureShadowShouldRedirect(name_id, syn)`, which decides by walking the actual
scope chain from `cur_scope` outward through `scope_nodes[...].parent`: at each scope, a declaration
of the original name in that scope suppresses the redirect (return false — a legal later same-name
local in a shallower sibling scope keeps its own variable); reaching the synth declaration's scope
node (`local_decl_scope_nodes`) redirects (return true). The two fallbacks (synth name not found /
synth scope not on the chain) keep the redirect, preserving the historical behavior. Pre-Task-10D
the guard used depth arithmetic over `local_decl_scopes`, which could not recognize a later
same-name local in a scope shallower than the synth capture's registration depth, so the stale
capture temp won (declaration + initializer dropped, uses aliased the old loop variable).
