...
`materializeInto(src_temp, expected, intent, src_node)` is the general mechanism: given a source temp and an expected type, it walks the type hierarchy (up to 8 optional/error-union layers) and emits `set_optional_null`/`wrap_optional`/`wrap_error_ok`/`wrap_error_err`, optionally preceded by an inner `int_cast`/`float_cast`. **Task 4 (pinned param/argument/return rule):** after the layers are unwound, a `value`-intent materialisation into an integer type calls `checkFoldedIntFits(self, src_node, cur)` — when the source expression node carries a folded integer (`KIND_INT`, `~` exempt) that does not fit the target exactly, it emits `error[3000] "comptime integer value does not fit the target type"` once per node (dummy temp; the post-lowering gate exits rc=2 with 0 `.c`). `applyCoercion` runs the same check on the recorded `(node_idx, target_type)` before dispatch, because `int_widen`/`int_literal_coerce`/`none` materialise without calling `materializeInto`; the module-init and local-decl `var_decl` slots and `return_stmt` call it directly (the decl slot type can differ from the fold's `@intCast`/`@as` target). **Task 4 untyped local slot:** an unannotated local binding whose init folds to an integer the inferred type cannot hold takes the value-based slot type (`comptimeIntUntypedType`) before the `decl_local` is emitted, so the slot and the HIT temp agree (`const c = 2000000000 + 1000000000;` is a `u32` slot).
