...
**Comptime-values HIT (fold consumer).** The `builtin_call` arm checks `ctx.comptime_values[node_idx]` before any runtime dispatch. On a hit the temp type defaults to `TYPE_USIZE`, is overridden for `@intCast` **and `@as`** (resolved target — the two builtins share the `[target_type, value]` AST layout, Task 11U; without the `@as` case a folded signed operand was materialised as an untyped/unsigned `TYPE_USIZE` constant, so `@as(i32,-2)` emitted `18446744073709551614ULL` and a binary operand computed wrong / trapped under `-fsafe`, Task 8B), `@isWindows` (`TYPE_BOOL`), and — Task 11D — `@floatCast`/`@intToFloat` (resolved `f32`/`f64` target), then emits `int_const` — except `@floatCast`/`@intToFloat`, whose map entry is an f64 bit pattern and which therefore emit `float_const` (the u64 is reinterpreted back to f64 via a pointer cast, since Z98 `@bitCast` is integer-only). A miss falls through to the runtime builtin forms below.
