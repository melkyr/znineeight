# 00 — Shared Infrastructure [updated: 2026-09-25 — Task 9 fix round 3 (z98-print-formatting Amendment 1, B5; operator ruling Q7): `ERR_3064` is RENAMED `ERR_3064_CYCLIC_GLOBAL_INIT=3064` (number unchanged) and now fires only for a cycle in the global-initializer dependency graph — emitted by `lower.zig`'s `lowerInitOrderVisit` (same-module cycle) and `main.zig`'s `computeModuleInitOrder` (cross-module cycle). The previous `ERR_3064_FORWARD_REF_TUPLE_GLOBAL` (a per-tuple reject in `semanticAnalyzerResolveTupleLiteral`) is deleted; forward references are now resolved by dependency-ordered `__module_init` emission. No `diagnosticCollectorMarkNodeOnce` dedupe is used for it.] [updated: 2026-09-25 — Task 9 (z98-print-formatting Amendment 1, B5): `diagnostics.zig` `ErrorCode` gains `ERR_3064_FORWARD_REF_TUPLE_GLOBAL=3064` (the new max code; emitted by `semantic_analyzer.zig`'s `semanticAnalyzerResolveTupleLiteral` when a recorded module-level tuple literal's re-resolved element list differs from the recorded one — a forward-referenced global frozen at the pass-1 `i32` fallback). `diagnosticCollectorMarkNodeOnce` dedupes it per tuple node.] [updated: 2026-09-25 — Task 3 (z98-print-formatting): `diagnostics.zig` `ErrorCode` gains `ERR_3063_PRINT_TYPE_NOT_SUPPORTED=3063` (the new max code; emitted by `lower.zig`'s print-format validator for a `print` argument with no `std.fmt` printer or an operator-ruled Q3 bounded residual — operator ruling Q1: 3058 is live `ERR_3058_CONDITION_NOT_BOOL`). `diagnosticCollectorMarkNodeOnce` now also dedupes the validator's per-argument-node diagnostics (`printFmtReject`, `lower.zig`)] [updated: 2026-09-22 — Task 6D: `diagnostics.zig` `ErrorCode` gains `ERR_3056_CALL_TARGET_NOT_CALLABLE=3056` (the max code; emitted by `lower.zig`'s generic call path for a non-callable lowered callee)] [updated: 2026-09-20 — refresh against the pool-backed growable-tier allocator, SourceManager fault-in, and config.zig/panic.zig coverage; line references and dated evidence removed]
...
| `diagnosticCollectorMarkNodeOnce` | pub | Per-node "already diagnosed" guard: linear scan of `diag_seen_items`; returns false if present, else records `node_idx` (growing the set, min 16, 2x) and returns true. | `type_resolver.zig` (array-size `ERR_3050` diagnostic); `lower.zig` `printFmtReject` (`ERR_3013`/`ERR_3063` print-format validator, per argument node) | `alloc_mod.sandAlloc` | Dedupes a node diagnosed by more than one resolution pass (e.g. front_resolution then sema/type resolution). | None [inference] |
...
| Function | Vis. | Purpose | Called By | Calls | Key Decisions | Markers |
|----------|------|---------|-----------|-------|---------------|---------|
| `readFile` | pub | Read entire file into arena memory. Copies path to null-terminated C buffer (max 511 bytes). Opens with `fopen` ("rb"), seeks to end for size, allocates arena buffer, reads content. Returns `?[]u8`. | import resolver (module source read into its own pool-backed `src_arena`), main root check, SourceManager `sourceManagerFaultIn` | `fopen`, `fseek`, `ftell`, `fclose`, `fread`, `sandAlloc` | Max path 511 bytes. Binary read. Arena allocation freed only by reset. OOM returns null (not panic). Null covers missing, empty (`ftell <= 0`), and oversize paths indistinguishably. | None [inference] |
| `fileExists` | pub | Check if a **regular file** exists by attempting `fopen("rb")`. Returns bool. Max path 511 bytes. | module resolution (candidate `.zig` files, `module_registry.zig:174,177`) | `fopen`, `fclose` | Opens and closes; no memory allocation. Returns true for empty files (can't distinguish empty). **Not** a directory probe — `fopen` on a directory fails on win32 (msvcrt). [updated: 2026-09-22 — Task 5B] | None [inference] |
| `dirExists` | pub | Check if a **directory** exists via `pal_dir_exists`. Max path 511 bytes. | CLI output-dir validation (`main.zig:164`) + default lib-dir lookup (`main.zig:437`) | `pal_dir_exists` | C-side directory probe (`GetFileAttributesA` win32 / `stat` POSIX). The default lib path `<exe_dir>/lib` names a directory, so its existence guard is this probe (Task 5B fix). [updated: 2026-09-22 — Task 5B] | None [inference] |
...
| Function | Vis. | Purpose | Called By | Calls | Key Decisions | Markers |
|----------|------|---------|-----------|-------|---------------|---------|
| `panicHandler` | pub | Print `"ICE: <msg> at <file>:<line>\n"` to stderr, then `pal.exit(3)`. | OOM in `sandAlloc`; `spill_store.zig`, `pal.zig` stream helpers, `module_registry.zig`, `ast.zig`, `lir_stream.zig`, `resolved_type_table.zig` | `pal.stderr_write`, `itoa_mod.itoa`, `pal.exit` | ICE exit code = 3. Formats line number via itoa. | None [inference] |
