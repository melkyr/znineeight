...
| `registerContainerType` | pub | `[inference: nameCacheGet("anon_<node>") short-circuit; typeRegistryRegisterNamedType(module 0, kind); setPacked for packed struct/union; resolve struct/union field types DIRECTLY (the module-only `resolveAggregateFieldTypesAll` never visits local/inline aggregates) and append the payload; delegate enum/error-set payloads to the shared `populateTypePayload`]` | **Task B2.** Registers one struct/enum/union/error-set container type and populates its payload; idempotent across passes. |
