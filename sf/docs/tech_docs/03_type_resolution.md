...
| `evalConstModuleOfExpr` | private | `[inference: resolve a field-access base to a module id, walking module aliases; 0 when not a module reference]` | Module-base resolver used by `evalConstIntFull`'s `field_access` arm. |
| `evalConstIntFull` | private | `[inference: exact internal fold over the Task-2 ComptimeInt core — int/char literals (ciFromU64), parens, negate (ciNeg), add/sub/mul/div/mod/bit_and/bit_or/bit_xor/shl/shr (ci*), ident chains (local const scope then all module symbol tables), field-access module consts, and the integer-valued builtins @intCast/@as (integer target + exact fit), @sizeOf/@alignOf/@bitSizeOf (complete types, struct completed via layoutEnsure), @offsetOf/@bitOffsetOf; depth-limit 16; returns ?ComptimeInt]` | **Task 5** (Task 1 §6.5): the ONE exact evaluator replacing the old 64-bit wrapping u32/i64 twins. `null` = unfoldable (unsupported shape, depth/cycle cap, div/mod by zero, >256-bit cap overflow, non-integer/non-fitting cast target). `~`, floats/bools, enum-member references and calls stay unfoldable here (the callers' ERR_3050/ERR_3055 paths own the rejects). |
| `evalConstU32Full` | pub | `[inference: evalConstIntFull + the preserved @intCast error[3000] arm + the 0..0xFFFFFFFE bound (evalConstIntToSize); falls back to the 0xFFFFFFFF sentinel]` | Array-size evaluator. **Task 11S (a)/Task 5:** the `@intCast` arm resolves the target, folds the operand exactly, and on a non-fit emits `error[3000]` (`@intCast value does not fit the target type`) once per node; a fit still passes the `0..0xFFFFFFFE` bound. **Task 4:** the fold is exact (no u32 wrapping), so `[0 - 1]u8` reaches `error[3050]`. **Task 5:** delegates everything else to `evalConstIntFull` (shifts/bitwise/parens and >64-bit intermediates now fold; `[1 << 40]u8` stays the sentinel — Z98's array length field is u32). |
| `evalConstI64Full` | pub | `[inference: evalConstIntFull materialised through [i64 min, u64 max] (comptimeIntFits64) into the two's-complement i64 pattern; null outside the window]` | Enum member-value evaluator. **Task 5:** exact — a wrapping initializer (`enum { A = 18446744073709551615 + 1 }`) is now `null` → `ERR_3055` instead of being stored as `0`; `@as`/`@intCast` targets use the shared exact fit. |
| `evalConstIntToSize` | private | `[inference: negative, over-0xFFFFFFFE, or the 0xFFFFFFFF sentinel collision -> the sentinel; else @intCast(u32, ciToU64(v))]` | **Task 5:** the `0..0xFFFFFFFE` array-size bound used by `evalConstU32Full`. |
| `enumMembersResolve` | pub | `[inference: one shared enum-member walk — fresh auto_val cascade; append vs overwrite; check_only keeps values in a local buffer; strict mode folds each explicit initializer via evalConstI64Full and rejects an unfoldable one (out_fail_kind=1) or a duplicate tag (out_fail_kind=2)]` | **Task 11J** (AMENDMENT 10): the ONE walk used by symbol registration (`append=true, strict=false`), the post-layout re-evaluation pass (`append=false, strict=true`), and the semantic analyzer's function-local enum check (`check_only=true, strict=true`). **Task 5:** the explicit-initializer fold is exact, so a 64-bit-wrapping value is rejected instead of silently stored. |
...
| `typeResolverCheckMemberVisibility` | private | `[inference: (owning module, pub flag) -> bool | emit error[3007]]` | **Task 15 (S3).** Cross-module `pub` gate for type positions and const-fold positions; called by `resolveTypeExprFull`'s `field_access` arm (ident-base + `module_type` sites) and by `evalConstIntFull`'s `field_access` arm (array sizes via `evalConstU32Full`, enum member values via `evalConstI64Full`). Returns true when `sym.module_id == env.module_id` or the symbol is `pub`; otherwise emits level-0 `error[3007]` `ERR_3007_VISIBILITY_VIOLATION` with the ASCII message `'<name>' is not marked 'pub'` (deduped per node) when `env.diag` is live, and returns false so the caller maps the expression to `TYPE_UNDEFINED` / declines the fold. Mirrors the semantic analyzer's `semanticAnalyzerCheckMemberVisibility`. |
| `isCompoundTypeExprKind` | pub | `[inference: kind in {ptr_type, many_ptr_type, array_type, slice_type, optional_type, error_union_type, fn_type}]` | **Task B2 final fix wave.** Predicate for the compound type-expression node kinds (`*E`/`[*]E`/`[N]E`/`[]E`/`?E`/`E!T`/`fn(...)T`); a local binding whose initializer is one names a `type` value, so the sema var-decl arm clean-rejects it rather than leak `TYPE_TYPE` into lowering. |
| `containerFieldCount` | private | `[inference: walk a struct/union node's extra children and count `field_decl` nodes]` | **Task B2 final fix wave.** Pre-count used to enforce `MAX_CONTAINER_FIELDS` before `registerContainerType` registers; module-scope `MAX_CONTAINER_FIELDS` (= 32) sizes the field scratch buffers. |
...
| `localConstScopeInit` / `localConstScopePush` / `localConstScopeLookup` | pub | `[inference: init/append/newest-first scan of the function-local const scope]` | `LocalConstScope` helpers used by `front_resolution.zig` and `evalConstIntFull` (the array-size/enum fold). |
...
| `field_access` | Resolve base expression. If base is `TYPE_UNDEFINED` and base is `ident_expr`, try module-qualified lookup (module symbol → field symbol). If base is `module_type`, look the field up in that module's symbol table. Emits `FAH:*` markers. **Task 15 (S3):** both module-member branches run `typeResolverCheckMemberVisibility` before using the symbol — a non-`pub` symbol owned by another module emits level-0 `error[3007]` `ERR_3007_VISIBILITY_VIOLATION` (`'<name>' is not marked 'pub'`, ASCII, deduped per node) and the arm returns `TYPE_UNDEFINED` (rc=2, 0 `.c`); same-module members are unchanged. |
...
| `array_type` | Resolve element type (`child_0`). Evaluate length from `child_1` via `evalConstU32Full` (every size expression kind — literal, arithmetic/bitwise/shift/parens, `ident_expr`, module-member `field_access`, integer-valued `builtin_call`). On success returns `typeRegistryGetOrCreateArray(elem, len)`; on failure emits `ERR_3050_ARRAY_SIZE_NOT_CONSTANT` (once per node) unless the size is the inferred `[_]` form. Emits `T0`, `T1e`, `T2L`, `T3a`, `A`/`a`. **Task 4:** the former inline u32-wrapping `add`/`sub`/`mul` arms are gone; the fold is exact and rejects negative/over-u32 results. **Task 5:** the exact fold is shared with the enum evaluator (`evalConstIntFull`), so shifts/bitwise/parens and >64-bit intermediates reduced back into range fold exactly. |

**Array-size evaluation:** the `array_type` arm's size node is folded by `evalConstU32Full` (module-member `field_access`, function-local consts, exact arithmetic/bitwise/shift/parens, and the integer-valued builtins `@intCast`/`@sizeOf`/`@alignOf`/`@bitSizeOf`/`@offsetOf`/`@bitOffsetOf` — Task 11H also folds a struct's `@sizeOf`/`@alignOf`/`@bitSizeOf` by completing it on demand via `layoutEnsure`). An uncomputable size is a hard `ERR_3050_ARRAY_SIZE_NOT_CONSTANT` diagnostic instead of a silent `TYPE_UNDEFINED`. **Task 4/5:** the fold is exact over the shared `ComptimeInt` core and the accepted range is exactly `0..0xFFFFFFFE` (the `0xFFFFFFFF` sentinel stays unfoldable), so `[0 - 1]u8` and `[4000000000 + 400000000]u8` reject (Zig: `type 'usize' cannot represent integer value '-1'`), while `[4 * 8]u8` / `[10 - 3]u8` / `[1 << 4]u8` / `[(2 + 1) * 2]u8` / `[(1 << 200) >> 190]u8` accept. An over-u32 exact size (`[1 << 40]u8`) is the sentinel (`ERR_3050`; Z98's array length field is u32, so this stays stricter than 64-bit Zig).

#### `evalConstIntFull`

`[inference: exact internal fold over the Task-2 `ComptimeInt` core, shared by both public evaluators, depth-limit 16, returns `?ComptimeInt`]` (Task 5, Task 1 §6.5 — replaces the two 64-bit wrapping twins):

1. **depth guard**: `depth > 16` → `null` (const-cycle guard, same cap as `resolveTypeExprFull`).
2. **int_literal / char_literal**: `ciFromU64(store.int_values[node.payload])` (the source value is already a u64).
3. **negate**: fold the child; `ciNeg` (`-0` normalises to `0`).
4. **paren_expr**: fold the child (transparent).
5. **add/sub/mul/div/mod_op/bit_and/bit_or/bit_xor/shl/shr**: fold both children and apply the exact `ci*` op; a cap-overflowing result (>256 magnitude bits), division/mod by zero, or a negative/out-of-domain shift count declines (`null`) — never a 64-bit wrap.
6. **ident_expr**: consult the function-local const scope first (`localConstScopeLookup`), then `symbolLookupAllModules`; a const symbol (`flags & 0x01 == 0`) recurses into `decl.child_1`.
7. **field_access**: resolve the base module via `evalConstModuleOfExpr`, then fold the member const's initializer. **Task 15 (S3) fix round 1:** the member symbol is gated by `typeResolverCheckMemberVisibility` first — a const owned by another module without the `pub` flag emits level-0 `error[3007]` `ERR_3007_VISIBILITY_VIOLATION` (`'<name>' is not marked 'pub'`, ASCII, deduped per node, only when `env.diag` is live) and the fold declines (`null`), so the caller's `ERR_3050` (array size) / `ERR_3055` (enum member) diagnostic also fires; `[mod.hidden_const]u8` and `enum(u8){A = mod.hidden_const}` previously folded the non-`pub` value silently.
8. **builtin_call**: the integer-valued builtins only:
   - `@intCast(T, e)` / `@as(T, e)` → resolve the target and fold the operand exactly; the target must be an integer type and the exact value must fit it (`comptimeIntFitsType`), else `null` (no diagnostic here — the enum caller's `ERR_3055` path owns its reject; the `evalConstU32Full` wrapper re-checks its `@intCast` arm with the preserved `error[3000]`);
   - `@sizeOf(T)` / `@alignOf(T)` / `@bitSizeOf(T)` → `resolveTypeExprFull` the type arg. If the resolved type is a not-yet-complete `struct_type` (packed or not), call the shared `layoutEnsure(registry, bt_tid, 0)` first, then re-read the type. Fold only if COMPLETE (`state == 2`) and (`evalConstScalarKind(kind)` — the integer whitelist — OR `struct_type`); return the registry `size` / `alignment` / bit-size (`@bitSizeOf`: `size*8`, overridden by `typeRegistryIntWidthBits` for integer/enum, `1` for bool, and `typeRegistryGetPackedTotalBits` for a packed struct). The `state == 2` gate is mandatory — reading `size`/`alignment` before layout produced a silently wrong `[1]` for an aggregate field;
   - `@offsetOf(T, "f")` / `@bitOffsetOf(T, "f")` → the resolved complete struct/packed-union field offset (bit offset for packed fields); a literal field name only;
   - `@isWindows`/`@intToFloat`/`@floatCast`, `~`, enum-member references, bools, and calls → `null` (the enum caller rejects `ERR_3055`; array positions `ERR_3050`). A forward-referenced/mutual aggregate also defers (`layoutEnsure` returns false on a `TYPE_VOID` placeholder or past the depth cap).
9. **Fallback**: `null` (unfoldable).

#### `evalConstU32Full` / `evalConstI64Full` (public wrappers)

- **`evalConstU32Full(env, node, depth) u32`** (array sizes): `null` → `0xFFFFFFFF`; a concrete value is accepted only in `0..0xFFFFFFFE` (negative/over-u32 and the `0xFFFFFFFF` collision are the sentinel). The `@intCast` arm keeps the Task 11S (a) diagnostic: resolve the target, fold the operand exactly, and on a non-fit emit `error[3000]` once per node (`@intCast value does not fit the target type`, `env.source_file_id`) and return the sentinel; on a fit the value still passes the `0..0xFFFFFFFE` bound (`evalConstIntToSize`). The result feeds the `array_type` arm, whose non-inferred failure emits the existing `ERR_3050_ARRAY_SIZE_NOT_CONSTANT`.
- **`evalConstI64Full(env, node, depth) ?i64`** (enum member values): materialises the exact value into the registry's i64 storage — a value outside `[i64 min, u64 max]` is `null` (`ERR_3055` at the enum walk), otherwise the two's-complement low 64 bits. The cast target check is the shared one. `~`, enum-member references, bool/float builtins, and calls stay `null`.
- `enumMembersResolve` (the shared strict member walk) and `enumReevaluateAll` (the post-layout re-evaluation pass) are structurally unchanged; their explicit-initializer fold now goes through the exact evaluator, so a wrapping initializer (`enum { A = 18446744073709551615 + 1 }`) is a clean `ERR_3055` instead of a silently stored `0`.

Helpers: `symbolLookupAllModules` — linear scan of all symbol tables for a `name_id`; `evalConstModuleOfExpr` — resolves a field-access base to a module id by walking module aliases; `evalConstScalarKind` — true for the primitive/alias kinds whose `@sizeOf`/`@alignOf`/`@bitSizeOf` are safe to fold (excludes struct/union/tagged-union/packed-union/tuple/array/slice/optional/error-union/fn/module/type/unresolved/none); Task 11H keeps it as the integer whitelist and adds `struct_type` as the only on-demand-completed aggregate; `evalConstIntToSize` (Task 5) applies the `0..0xFFFFFFFE` array-size bound.
...
5. **`evalConstU32Full` fallback ambiguity (pre-existing):** `0xFFFFFFFF` is both a valid `u32` and the "uncomputable" sentinel, so an array of length `0xFFFFFFFF` cannot be distinguished from a failed fold. **Task 5:** the evaluator delegates to the shared exact `evalConstIntFull` (int/char literals, parens, `negate`, the ten integer binops, ident/module-const chains, field-access module consts, and the integer-valued builtins `@intCast`/`@as`/`@sizeOf`/`@alignOf`/`@bitSizeOf`/`@offsetOf`/`@bitOffsetOf`); a cap-overflow/div-by-zero/shift-domain decline or a non-integer/non-fitting cast target falls back to the sentinel. `@isWindows`/float builtins, `~`, and every non-struct aggregate kind still fall back to the sentinel (`ERR_3050`); a forward-referenced/mutual aggregate defers. The accepted concrete range is exactly `0..0xFFFFFFFE`.
