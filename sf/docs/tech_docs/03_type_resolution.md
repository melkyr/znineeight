...
| `array_type` | Resolve element type (`child_0`). Evaluate length from `child_1` via `evalConstU32Full` (every size expression kind — literal, arithmetic, `ident_expr`, module-member `field_access`, integer-valued `builtin_call`). On success returns `typeRegistryGetOrCreateArray(elem, len)`; on failure emits `ERR_3050_ARRAY_SIZE_NOT_CONSTANT` (once per node) unless the size is the inferred `[_]` form. Emits `T0`, `T1e`, `T2L`, `T3a`, `A`/`a`. **Task 4:** the former inline u32-wrapping `add`/`sub`/`mul` arms are gone; the fold is exact and rejects negative/over-u32 results. |

**Array-size evaluation:** the `array_type` arm's size node is folded by `evalConstU32Full` (module-member `field_access`, function-local consts, arithmetic, and the integer-valued builtins `@intCast`/`@sizeOf`/`@alignOf`/`@bitSizeOf` — Task 11H also folds a struct's `@sizeOf`/`@alignOf`/`@bitSizeOf` by completing it on demand via `layoutEnsure`). An uncomputable size is a hard `ERR_3050_ARRAY_SIZE_NOT_CONSTANT` diagnostic instead of a silent `TYPE_UNDEFINED`. **Task 4:** the accepted range is exactly `0..0xFFFFFFFE` (the `0xFFFFFFFF` sentinel stays unfoldable), so `[0 - 1]u8` and `[4000000000 + 400000000]u8` reject (Zig: `type 'usize' cannot represent integer value '-1'`), while `[4 * 8]u8` / `[10 - 3]u8` accept.
...
2. **int_literal**: the stored u64 value must be `<= 0xFFFFFFFE` (a larger literal, including the `0xFFFFFFFF` sentinel collision, is unfoldable) — Task 4.
3. **add/sub/mul/div/mod_op**: evaluate both children; if both are known, compute EXACTLY in u32 (Task 4): an `add` over `0xFFFFFFFE`, a `sub` below zero, a `mul` over `0xFFFFFFFE` (or the sentinel product), and div/mod by zero are `0xFFFFFFFF`; negative results are never wrapped.
4. **negate**: `-0` → `0`; any other operand is unfoldable (a negative array size rejects) — Task 4.
