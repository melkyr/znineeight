    ...
    return result;
        ...
        if (cond_ty_id) |ct| {
            ...
            if (ct_ty.kind == type_mod.TypeKind.tagged_union_type) {
                ...
                emitInst(self, LirInst{ .load_field = .{ .name_id = @intCast(u32, 0), .base = cond_temp, .field_id = @intCast(u32, 0), .result = tag_temp } });
                cond_temp = tag_temp;
            }
        }
...
pub fn lowerStmt(self: *LirLowerer, node_idx: u32) void {
    ...
    if (node.kind == AstKind.block) {
        ...
        var mwpb: [10]u8 = undefined; var mwpl = itoa_mod.itoa(node.payload, mwpb[0..]); var mwps: usize = @intCast(usize, 9) - @intCast(usize, mwpl); pal.markerWrite(mwpb[mwps..@intCast(usize, 9)]);
