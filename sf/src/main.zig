...
fn resolveDeclAggregateFieldTypes(ctx: *CompilerContext, mod_id: u32, decl_idx: u32) void {
    var decl = ctx.store.nodes.items[@intCast(usize, decl_idx)];
    var init = ctx.store.nodes.items[@intCast(usize, decl.child_1)];
    var spid = type_mod.nameCacheGet(ctx.typereg, (@intCast(u64, mod_id) << @intCast(u64, 32)) | @intCast(u64, decl.payload));
    if (spid) |stid| {
        ...
        while (di < decls.len) : (di += 1) {
            ...
            if (decl.kind == AstKind.var_decl and decl.child_1 != 0) {
                ...
                if (init.kind == AstKind.struct_decl or init.kind == AstKind.union_decl) {
                    resolveDeclAggregateFieldTypes(ctx, mods[mi].id, decls[di]);
                } else {
...
fn resolveTypeExprDepth(ctx: *CompilerContext, node_idx: u32, depth: u32) type_mod.TypeId {
    ...
    if (node.kind == AstKind.ident_expr) {
        ...
        if (tid) |t| return t;
        var nf: []const u8 = "NF"; pal.markerWrite(nf);
        var mi: usize = 0;
    ...
    if (node.child_0 != 0) {
        ...
        if (node.kind == AstKind.ptr_type or node.kind == AstKind.many_ptr_type) {
            var ptm: []const u8 = "PTR:i"; pal.markerWrite(ptm);
            var ptib: [10]u8 = undefined; var ptil = itoa_mod.itoa(node_idx, ptib[0..]); var ptis: usize = @intCast(usize, 9) - @intCast(usize, ptil); pal.markerWrite(ptib[ptis..@intCast(usize, 9)]);
            var ptkm: []const u8 = "k"; pal.markerWrite(ptkm);
            var ptkb: [10]u8 = undefined; var ptkl = itoa_mod.itoa(@intCast(u32, @enumToInt(node.kind)), ptkb[0..]); var ptks: usize = @intCast(usize, 9) - @intCast(usize, ptkl); pal.markerWrite(ptkb[ptks..@intCast(usize, 9)]);
            var ptcm: []const u8 = "c"; pal.markerWrite(ptcm);
            var ptcb: [10]u8 = undefined; var ptcl = itoa_mod.itoa(child_type, ptcb[0..]); var ptcs: usize = @intCast(usize, 9) - @intCast(usize, ptcl); pal.markerWrite(ptcb[ptcs..@intCast(usize, 9)]);
            var is_const: bool = (node.flags & @intCast(u8, 1)) != @intCast(u8, 0);
            if (node.kind == AstKind.ptr_type) {
                var ptr_tid = type_mod.typeRegistryGetOrCreatePtr(ctx.typereg, child_type, is_const);
                var ppm: []const u8 = "P"; pal.markerWrite(ppm);
                var ppb: [10]u8 = undefined; var ppl = itoa_mod.itoa(ptr_tid, ppb[0..]); var pps: usize = @intCast(usize, 9) - @intCast(usize, ppl); pal.markerWrite(ppb[pps..@intCast(usize, 9)]);
                var pnl: []const u8 = "\n"; pal.markerWrite(pnl);
                return ptr_tid;
            } else {
                var ptr_tid2 = type_mod.typeRegistryGetOrCreateManyPtr(ctx.typereg, child_type, is_const);
                var ppm2: []const u8 = "M"; pal.markerWrite(ppm2);
                var ppb2: [10]u8 = undefined; var ppl2 = itoa_mod.itoa(ptr_tid2, ppb2[0..]); var pps2: usize = @intCast(usize, 9) - @intCast(usize, ppl2); pal.markerWrite(ppb2[pps2..@intCast(usize, 9)]);
                var pnl2: []const u8 = "\n"; pal.markerWrite(pnl2);
                return ptr_tid2;
            }
        }
        if (node.kind == AstKind.slice_type) {
            ...
            var sm: []const u8 = "SL:e"; pal.markerWrite(sm); pal.markerWrite(sl_e[sl_es..@intCast(usize, 19)]);
            var sl_r: [20]u8 = undefined;
            var sl_rl = itoa_mod.itoa(sl_tid, sl_r[0..]);
            var sl_rs: usize = @intCast(usize, 19) - @intCast(usize, sl_rl);
            var s2: []const u8 = "s"; pal.markerWrite(s2); pal.markerWrite(sl_r[sl_rs..@intCast(usize, 19)]);
            var s3: []const u8 = "\n"; pal.markerWrite(s3);
            return sl_tid;
        }
