...
fn testSignatureVoidParam() void {
    ...
    var proto = ast_mod.FnProto{ .name_id = @intCast(u32, 0), .params_start = @intCast(u32, store.extra_ranges.items[@intCast(usize, param_payload)] >> 32), .params_count = @intCast(u16, 1), .call_conv = @intCast(u8, 0), .return_type_node = @intCast(u32, 0) };
