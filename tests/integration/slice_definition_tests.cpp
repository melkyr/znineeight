...
TEST_FUNC(SliceDefinition_PrivateFunction) {
    ...
    if (f == PLAT_INVALID_FILE) {
        printf("FAIL: Failed to open generated zig_special_types.h\n");
        return false;
    }
    ...
    bool found = (strstr(content, "Slice_u8") != NULL);
...
TEST_FUNC(SliceDefinition_NestedType) {
    ...
    // Check zig_special_types.h content
    ...
    bool found_i32 = (strstr(content, "Slice_i32") != NULL || strstr(content, "Ptr_Slice_i32") != NULL);
    bool found_f32 = (strstr(content, "Slice_f32") != NULL);
