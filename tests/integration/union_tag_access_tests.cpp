...
TEST_FUNC(UnionTagAccess_Basic) {
    ...
    if (!unit.validateVariableEmission("tag", "enum Value_Tag tag = Value_Tag_Int;")) {
        return false;
    }
...
TEST_FUNC(UnionTagAccess_Alias) {
    ...
    if (!unit.validateVariableEmission("t1", "enum Value_Tag t1 = Value_Tag_A;")) return false;
    if (!unit.validateVariableEmission("t2", "enum Value_Tag t2 = Value_Tag_B;")) return false;
...
TEST_FUNC(UnionTagAccess_Imported) {
    ...
    if (!unit.validateVariableEmission("t", "enum JsonValue_Tag t = JsonValue_Tag_Null;")) {
        return false;
    }
