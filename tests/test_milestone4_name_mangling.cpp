...
TEST_FUNC(NameMangler_Milestone4Types) {
    ...
    Type* err_set = createErrorSetType(unit, unit.getBuiltinModule(), NULL, tags, true);
    ASSERT_STREQ("errset_A_B", mangler.mangleType(err_set));

    // Named Error Set: MyError -> MyError
    Type* named_err_set = createErrorSetType(unit, unit.getBuiltinModule(), interner.intern("MyError"), tags, false);
