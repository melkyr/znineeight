...
The C89 Compiler Validation Framework is a test-only utility designed to ensure that the C code generated by the Z98 compiler is strictly compliant with the ANSI C89 standard and adheres to the specific constraints of MSVC 6.0.

## Overview

As Z98 targets 1998-era hardware and software, it is critical that the emitted C code does not accidentally use modern C features (C99, C11, etc.). The validation framework provides two layers of verification:
