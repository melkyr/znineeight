...
- Compile-time comparisons: the Task 9D bounded divergence is RETIRED (Task 3). Comparisons of `comptime` integers are signedness-free and exact over the value's magnitude+sign (within the 256-bit cap), so arithmetic-derived conditions such as `(umax - 1) > 0`, `0 < (umax - 1)`, `(umax - 1) > zero`, `umax > (0 + 0)`, `(a + 1) == 2` and `(imin + 1) < 0` fold like official Zig; the condition of a capture-free no-`else` value `if` is stored so the lowered branch matches Zig even for module-scope operands (including a then-arm that returns, Task 3 fix rounds 1-2). Arithmetic folds over a declared integer operand apply a peer-fit rule (an untyped operand and the exact result must fit the peer type — including the unary `-`, nested arithmetic, and a name typed through an unannotated `const`'s initializer), so Zig-rejected shapes (`u - 300` on a `u8`, `0 - umax` on a `u64`, `-umax` on a `u64`) stay rejected; `~` is exempt (the pre-existing Z98 `~x = -x-1` divergence from Zig's wrapped complement — the accepted `(~u) != 0` class is runtime-equal, while a shape depending on the wrapped value can false-reject). `umax < 0` and `(umax - 1) < 0` fold false and are rejected as before. Float comparison folding remains out of scope. (Fixtures: `repro/mi_matrix/stdlib_comptime_compare_xmod`, `repro/mi_matrix/comptime_compare_reject_xmod`, `repro/mi_matrix/stdlib_comptime_noreturn_if_xmod`; full semantics text follows in the Task 5 consumer migration.)
