...
- **No Method Syntax**: `struct.func()` is not supported; use `func(struct)`. (Exception: a call whose callee is named `print` gets format-string lowering; see §4.) A member not found on a struct/union/tagged-union value — which is always the case for `struct.func()`, since Z98 aggregate types cannot contain function declarations — is rejected at compile time with `error[3060]` (`no field or member function named '<name>' in struct/union type`); see `sf/docs/tech_docs/05_semantic_analysis.md` §semanticAnalyzerResolveFieldAccess Phase 5.
