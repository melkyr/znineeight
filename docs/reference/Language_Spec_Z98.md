...
- **Unions**:
    ...
    - **Tagged Unions**: `const U = union(enum) { field: T, ... };`. Automatically managed tag and payload.
        ...
        - **Note**: The bootstrap compiler currently has a known bug where it may incorrectly use the `union` keyword in C declarations for tagged unions (which are emitted as C `struct`s).
...
- `switch (expr) { ... }`: Pattern matching and conditional evaluation.
  ...
  - **Ranges**:
    ...
    - **Note**: Character literals (e.g., `'0'...'9'`) are currently NOT supported as range bounds in the bootstrap compiler.
...
- **Recursive Types**: Defining a type that contains a slice of itself (e.g., `struct { v: []Self }`) is currently limited and may trigger "incomplete type" errors.
- **Multi-Module Stability**: Symbol resolution across modules via `@import` is currently unstable for complex dependency graphs.
