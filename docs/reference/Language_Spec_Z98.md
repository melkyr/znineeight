...
| any other type / a `type`-valued expression (e.g. `. {u32}`) | 3063 | 3063 | 3063 | 3063 |
    ...
    - **Documented bounded residuals of this program** (all Zig-0.15.2-accepted unless stated; each is a deliberate clean reject or a pre-existing limitation; the print route has no silent wrong value since the exact untyped-literal fix above):
        ...
        - **Forward-referenced tuple-global elements (Task 9, 2026-09-25)**: a module-level tuple literal whose element references a global declared LATER (`var g = .{ b, 7 }; const b = Pair{ a: i32, b: i32 }...` and the scalar variants `const b = 5` / `const b = 3000000000`) clean-rejects with the new level-0 `error[3064]` (`cannot infer tuple element type: a forward-referenced global is not resolved on the first pass`), rc=2 / 0 `.c`. Pass 1 of the module-var resolution loop resolves the tuple before the later global is typed, so the element freezes at the `TYPE_VOID -> TYPE_I32` fallback; reusing the recorded (idempotent) tuple type emitted gcc-invalid C (`zT._0 = zG_b` assigning a `Pair` into an `int`) or a silently wrong value (`3000000000` -> `-1294967296`). `__module_init` lowers globals in declaration order, so a type refresh alone cannot make the composite case runtime-correct; the shape is clean-rejected instead of emitting broken or silently wrong C. Zig 0.15.2 accepts and prints it (fixture `repro/mi_matrix/tuple_fwd_global_reject_xmod` + standalone `repro/print_tuple_fwd.z98`).
