...
| any other type / a `type`-valued expression (e.g. `. {u32}`) | 3063 | 3063 | 3063 | 3063 |
    ...
    - **Documented bounded residuals of this program** (all Zig-0.15.2-accepted unless stated; each is a deliberate clean reject or a pre-existing limitation; the print route has no silent wrong value since the exact untyped-literal fix above):
        ...
        - **Forward-referenced tuple-global elements (Task 9, 2026-09-25; reject narrowed in fix round 1)**: a module-level tuple literal whose element references a global declared LATER is resolved on a later module-var pass than the tuple (pass 1 freezes the element at the `TYPE_VOID -> TYPE_I32` fallback). The **broken** shapes clean-reject with the level-0 `error[3064]` (`cannot infer tuple element type: a forward-referenced global is not resolved on the first pass`), rc=2 / 0 `.c`: a composite/pointer/float/bool element (reusing the frozen `int` slot emitted gcc-invalid C, `zT._0 = zG_b` assigning a `Pair` into an `int`, or silently wrong values), an integer whose exact value does not fit `i32` (`const b = 3000000000` printed `-1294967296`), and a non-literal scalar init (`5 + 7`, `-5`, `true`, `1.5` lower to a global load that `__module_init` stores AFTER the tuple owner, so they printed `0`/`0`/`0`/`1`). The **one benign class stays accepted and prints Zig-identically**: a module `const` with a bare int/char literal init whose exact value fits `i32` (`const b = 5`, `const b = 2147483647`, `const b = 'A'`) — the literal is inlined at the use site (the module-init emitter skips literal consts), so the frozen slot holds it exactly. `__module_init` lowers globals in declaration order, so a type refresh alone cannot make the broken cases runtime-correct; a true fix needs dependency-ordered module-init emission (out of this round). Zig 0.15.2 accepts and prints every row. Reject fixture `repro/mi_matrix/tuple_fwd_global_reject_xmod` (6 x `error[3064]`), positive fixture `repro/mi_matrix/stdlib_print_tuple_fwd_ok_xmod`, standalone `repro/print_tuple_fwd.z98`.
