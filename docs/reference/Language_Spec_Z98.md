...
| any other type / a `type`-valued expression (e.g. `. {u32}`) | 3063 | 3063 | 3063 | 3063 |
    ...
    - **Exact untyped-literal values** (final whole-branch review fix wave, 2026-09-25): a `{}`/`{d}`/`{x}` argument whose static type is `integer_literal` is materialised at lowering into the carrier of its exact value (`i32` when it fits, else `u32`/`i64`/`u64`; literal-only arithmetic is folded exactly the same way the unannotated-local slot type already is), so `print("{}", .{3000000000})` -> `3000000000`, `{x}` -> `b2d05e00`, `0 - 3000000000` -> `-3000000000`, and `18446744073709551615` -> `18446744073709551615` (each equal to the Zig 0.15.2 twin). Values that fit `i32` keep the pre-fix temp and emit byte-identical C; a literal-only exact value outside the 64-bit window is a clean `error[3000]` (the documented no->64-bit-runtime-slot bound). Fixture `repro/mi_matrix/stdlib_print_untyped_lit_xmod`.
    ...
    - **Documented bounded residuals of this program** (all Zig-0.15.2-accepted unless stated; each is a deliberate clean reject or a pre-existing limitation; the print route has no silent wrong value since the exact untyped-literal fix above):
        ...
        - **R11** (operator ruling, final whole-branch review fix wave 2026-09-25): a recursive aggregate (`struct Node { v: i32, next: *Node }`) or a mutually recursive pair (`A { b: *B }` / `B { a: *A }`) rejects `{}` with `error[3063]`; Zig 0.15.2 accepts and prints the nested `.{ .next = .{ ... } }` form (a cycle terminates at `std.fmt.default_max_depth` = 3). Latent emitter risk recorded with the residual: `emitAggPrinterRec` emits printers in post-order with no forward declarations, so relaxing the validator would first need forward declarations plus a recursion strategy (a self-cycle would not terminate; a mutual `A -> B -> A` cycle has no valid post-order). Fixture `repro/mi_matrix/print_recursive_aggregate_reject_xmod`.
        ...
        - **Forward-referenced tuple-global elements (Task 9, 2026-09-25; reject narrowed in fix round 1, class-complete in fix round 2)**: a module-level tuple literal whose element references a global declared LATER is resolved on a later module-var pass than the tuple (pass 1 freezes the element at the `TYPE_VOID -> TYPE_I32` fallback). Every recorded element is validated: a value is **benign** when it is INLINED at the use site — a module `const` with a bare int/char literal init whose exact value fits the recorded slot (`const b = 5`, `2147483647`, `'A'`, `const b: u32 = 5`) — or when the global is initialised BEFORE the tuple (an earlier same-module declaration, e.g. `const back: i32 = 5 + 7;` above the tuple; an earlier module). **All other shapes clean-reject** with the level-0 `error[3064]` (`cannot infer tuple element type: a forward-referenced global is not resolved on the first pass`), rc=2 / 0 `.c`: a composite/pointer/float/bool element (reusing the frozen `int` slot emitted gcc-invalid C, `zT._0 = zG_b` assigning a `Pair` into an `int`), an integer whose exact value does not fit `i32` (`const b = 3000000000` printed `-1294967296`), a non-literal scalar init (`5 + 7`, `-5`, `true`, `1.5` lower to a global load that `__module_init` stores AFTER the tuple owner, so they printed `0`/`0`/`0`/`1`), the **same-type** variants of that class (`const b: i32 = 5 + 7`, `-5`, or `@as(i32, 5)`: the type does not change across passes but the value store still runs later — fixed in round 2), a cross-module non-literal const (the root module's `__module_init` runs before imported modules'), and a direct `@import("x.zig").C` member reference (the lowerer does not inline this form; the base emitted gcc-invalid C). References are recognised through `( ... )` chains, aliased module members (`colors.C`), and global aggregate fields. `__module_init` lowers globals in declaration order, so a type refresh alone cannot make the broken cases runtime-correct; a true fix needs dependency-ordered module-init emission (out of this round). Zig 0.15.2 accepts and prints every row. Reject fixture `repro/mi_matrix/tuple_fwd_global_reject_xmod` (11 x `error[3064]`), positive fixture `repro/mi_matrix/stdlib_print_tuple_fwd_ok_xmod` (9 rows, byte-identical to the Zig twin), standalone `repro/print_tuple_fwd.z98` (3 x `error[3064]`). **Residual sub-class:** a forward global reference hidden inside a CALL ARGUMENT (`var g = .{ id(fwd), 7 };` with `fwd: i32 = 5 + 7` declared later) is not walked — call-argument lists live in the AST's `extra_children` pool, and a generic extras walk is unsafe — so it is accepted and silently reads the pre-init value (`g = .{ 0, 7 }` where Zig prints `12`); a kind-guarded extras descent is left to a future round.
