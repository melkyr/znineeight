...
Slices (`[]T`) are emitted as C structures. For each unique slice type encountered, the `CBackend` generates a `typedef` and a corresponding constructor helper.

| Zig Type | C89 Representation |
|----------|-------------------|
| `[]i32`  | `Slice_i32` (struct) |
| `[]const u8` | `Slice_const_u8` (struct) |
...
### 1.3 64-bit Suffixes and Compatibility
...
Slicing expressions `base[start..end]` are emitted as calls to static inline helper functions generated by the backend.

| Zig Syntax | C89 Emission |
|------------|--------------|
| `arr[0..5]`| `slice_i32(&arr[0], 5)` |
| `s[1..]`   | `slice_i32(s.ptr + 1, s.len - 1)` |

### 11.3 Dereference Syntax
