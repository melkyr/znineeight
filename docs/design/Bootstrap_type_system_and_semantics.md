...
-   **Static Variant Access**: Support for accessing variant names on tagged union types (e.g., `Value.Cons`) is implemented, allowing type unwrapping and constant folding. The `TypeChecker::visitMemberAccess` function distinguishes static type access (which resolves to the tag enum value) from instance access.
...
1. **Topological Sorting (Task 235)**: After all modules are loaded and their imports resolved, the `CompilationUnit` performs a topological sort using Kahn's algorithm. This ensures that modules are type-checked in an order that respects their dependencies, providing a stable and predictable state for cross-module type resolution.
2. **Placeholder Types**: When a type definition (`const T = struct { ... }`) is encountered, a `TYPE_PLACEHOLDER` is immediately registered in the symbol table.
3. **Pre-registration Pass**: Before full type checking, a pre-scan of all modules (now in topological order) registers placeholders for all top-level types. This ensures that cross-module mutual recursion (Module A uses Type B, Module B uses Type A) works correctly.
...
- **Member Access**: The internal fields of the optional struct can be accessed directly using dot notation:
    - `opt.has_value`: A `bool` indicating if a value is present (0 for null, 1 for present).
    - `opt.value`: The underlying payload of type `T`.
    - **Restriction**: These members are **read-only**. Assigning to `opt.value` or `opt.has_value` is a compile-time error.
    - **Void Payload**: Accessing `.value` on an optional with a `void` payload (`?void`) is a compile-time error.
- **Null Comparison**: Optional types can be compared with the `null` literal using `==` and `!=`. This is functionally equivalent to checking the `has_value` member.
