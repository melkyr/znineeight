...
- **Implicit Coercion**: Array-to-slice coercion is automatically handled by the `TypeChecker` by inserting a synthetic slicing node, which the emitter then translates into a helper call. String literals (typed as `*const [N]u8`) are also implicitly coerced to `[]const u8` slices or `[*]const u8` many-item pointers. For many-item pointers, the coercion logic decays the array pointer to its first element (e.g. `&(*ptr)[0]` in C). For control-flow expressions yielding slices, the TypeChecker uses a **Distributed Coercion** strategy to ensure branches produce structs, not pointers, before lifting occurs.
