...
### Windows (`-osw`) build — Win9x target; every `std` importer links wsock32  [added: 2026-09-10 — STDLIB net re-export]

Since `sf/src/std.zig` re-exports `net` **unconditionally**, EVERY bare `@import("std")` consumer emits `std_net` and — under the `-osw` (windows) target — references the Windows socket API, **even if the program never uses net**.
- `net_prelude.h` is a **COMMITTED** header at `sf/src/include/net_prelude.h` (NOT generated by zig1). The emitted `std_net_*.h` does `#include <net_prelude.h>`; it is found via `-I sf/src/include` (the same include path already used for `zig_runtime.h`). Under `_WIN32` it pulls `<winsock.h>`; otherwise the POSIX socket headers.
- **PROBE (2026-09-10, never-net `repro/mi_matrix/stdlib_str_xmod` = bare `@import("std")`):** `-osw` dump rc=0, 9 `.c`; mingw `-c` all clean. Link **WITHOUT** `-lwsock32` **FAILS** (`undefined _imp__recv@16` / `_imp__closesocket@4` / `_imp__select@20`); link **WITH** `-lwsock32` **rc=0**. So `-lwsock32` is REQUIRED for any `-osw` std importer.
```bash
cd /workspace/znineeight
# 1) dump with the windows target flag (-osw; -osl is the linux default)
timeout 120 <zig1> -osw --dump-c89 --output-dir <dump> <entry.zig>
# 2) compile each emitted .c  (-I sf/src/include carries net_prelude.h)
for f in <dump>/*.c; do i686-w64-mingw32-gcc -std=c89 -m32 -Wall -Wno-long-long \
    -Wno-pointer-sign -I sf/src/include -c "$f" -o "${f%.c}.o"; done
# 3) link  (-lwsock32 REQUIRED for any std importer; net_prelude.h is not generated)
i686-w64-mingw32-gcc -m32 -o <exe> <dump>/*.o sf/src/include/zig_runtime.c \
    sf/src/include/zig_pal.c sf/src/c_exit.c -lwsock32
```
- Harness wrapper: `scripts/win32_cross/cross_build_run.sh <zig1> <entry> <workdir> <exe> [-lwsock32]` (add `-lwsock32` for std importers; the script appends extra libs to its link line).
