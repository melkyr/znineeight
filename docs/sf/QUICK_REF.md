...
### Corpus gate (322 dirs in `repro/mi_matrix/*/`, all with `main.zig`)  — classify by gcc EXIT CODE  [updated: 2026-08-24 — R2/R1 residual closeout GATE]
...
- **Out-of-scope residual closeout — Phase 5 progress (2026-08-24, mid-plan; final reconciliation lands at GATE-FINAL):** corpus 323 dirs now `OK=313 FAIL=1 ICE=0 CRASH=0 GREEN=9`. FAIL=9→1: plat_stubs CRASH→OK (F-PLATSTUBS `7d1b512d`), orelse/catch labeled-stmt →OK (F-LABELED `2cbf1fd3`), `emission_opt_fptr_wrap_xmod` added+OK (F-OPTFPTR `53b9f1b6`), `parsergap_specifier_xmod` {x}-hex →OK (F-SPECIFIER `a9ea91f0`), `strictzig_brace_if_xmod`/`parsergap_selfblok_xmod`/`parsergap_strict_comma_xmod`/`parsergap_slice_expr_xmod` → green-guards (reclassified). **`field_store_drop` →OK (operator-directed fixture correction `3ef2e8c8`):** its bare `@import("pal")` masked the fixture with `error[3048]`; corrected to `@import("pal.zig")` + bundled `pal.zig` stub (`@stderrWrite`) + `std.io.printInt` sinks (the `__bootstrap_print_int` extern was F4-removed). Now dump/gcc/link/run all rc=0 (stdout `2030`, stderr `OK`); the emitted `store()` shows direct `t.s=s;t.b=b;t.c=c;` — the documented temp-0 sentinel extra-copy symptom is GONE, superseded by later compiler fixes. Remaining FAIL=1 = `self_embed_optional_cycle` (error[24], genuine deferred design gap — optional-by-value can't break the self-cycle; decision-gate Task 5.6). 4 MD5 gates byte-identical at json `089e4f04…` (AMENDMENT 4 re-baseline).
