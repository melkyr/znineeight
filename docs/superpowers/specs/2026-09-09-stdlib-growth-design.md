...
- **Corpus/Gates (same as every plan):** current corpus state (post-EMITCOMPACT, seed v6, fixed point `ea149e05`) = 420 dirs = 404 OK / 9 GREEN / 7 FAIL. Each stdlib increment: corpus zero-asymmetric except the new fixture dirs (absent → OK/GREEN). Golden 9/9 + matrix 21/21 run byte-identity. 4-MD5 dump gates (gol `bbafa30f…`, lisp `c1767529…`, json `3ada7d8b…`, mud `ee42f9b7…`) are UNCHANGED by stdlib work unless a gate *program* imports a changed std module — recorded-not-rebaselined if so, operator-ruled re-baseline only at closeout.
