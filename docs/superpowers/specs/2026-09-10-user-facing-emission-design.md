...
The emit phase writes into `DIR`: all reachable module `.c/.h`, `zig_special_types.h`, `zig_compat.h`, `zig_runtime.h`, `net_prelude.h`, `zig_runtime.c`, `zig_pal.c`, `c_exit.c`, plus the companion scripts. **Sourcing = a hand-written Z98 compiler helper module** (`sf/src/emit_support.zig`) holding the canonical bytes of `sf/src/include/*` + `sf/src/c_exit.c` as Z98 string constants and exposing the emit functions; the compiler imports it and invokes it at emission time (the compiler itself emits the files — one self-sufficient app, like a std-kind helper), with a `.sh` byte-equality gate against the canonical files (Task-1 decision 1). **No host-side generator, no `perl`, no generated source module, no script binding other than `.sh`/`.bat`** (operator directive 2026-09-10; the reverted `0cca9158` used a perl generator and was rejected). The dead `emitZigPalC`/`emitZigRuntimeC` mirrors' stale CONTENT must NOT be revived — they are stale relative to `sf/src/include` (missing `errno.h` + `pal_get_default_lib_path`, different runtime body); Task 4 embeds canonical bytes and adds the equality gate.
