...
> **AMENDMENT 5 (2026-08-24, operator ruling after GATE-FINAL sweep):** GATE-FINAL's corpus sweep found `repro/mi_matrix/plat_stubs_missing_xmod` — documented OK in the v44 baseline (OK=292/303) — deterministically SEGVs at HEAD (ASan, `resolveStmtTypes` front_resolution.zig:131, 0 `.c`, dump rc=1). Bisected: **pre-existing, NOT caused by this plan** (a fresh build at plan-start HEAD `b9256f2e` crashes identically; last-known-good ~Aug 18 `f4_fix_zig1`; suspected window = silent-drop u32-widening `378c71fa`/`50ebbf82`, unverified). Prior gate sweeps mis-recorded it as OK. **OPERATOR RULING: record it truthfully and proceed.** GATE-FINAL reconciles the docs to the true corpus state: **322 dirs** (303 baseline + 7 new 194-plan dirs + 10 A-ADD dirs + R-R2 `emission_temp_index_drift_xmod` + R-R1 `emission_opt10_assign_xmod`), **OK=307 / FAIL=9** (the documented 7 + `emission_orelse_labeled_xmod` + `emission_catch_labeled_xmod` — the po1/pco1 labeled-stmt classes remain RED; Fix A did NOT close them, separate live bugs outside this plan's fix scope) **/ CRASH=1** (`plat_stubs_missing_xmod`, pre-existing, documented with the mis-recording note) **/ ICE=0 / green-guards=5** (4 documented + `emission_pal_xmod` error[20] per AMENDMENT 1). No new fix task.
