...
- [ ] **Step 4: GREEN + safety/determinism gates + fixed point UNMOVED + commit.**
...
- [ ] **Step 4: GREEN + arena gate + safety/determinism gates + fixed point UNMOVED + commit.**
...
**Files:**
- Modify: `scripts/seed/build_from_seed.sh`, `scripts/seed/archive_seed.sh`, `repro/mi_matrix/EXPECTED_FAIL.md`, `docs/sf/QUICK_REF.md`, `release/seed/CHANGELOG.md`

- [ ] **Step 1: Extend the seed scripts' `lib/` copy list** with `std_crypto.zig`/`std_parse.zig`/`std_map.zig`/`std_sort.zig`/`std_heap.zig`/`std_rle.zig`/`std_base64.zig`/`std_hex.zig`/`std_utf8.zig`.
- [ ] **Step 2: Run the full corpus + gates** (count; `check_emit_support` 5/5; `CLOSEOUT OK`; zero class movement on pre-existing dirs).
- [ ] **Step 3: Bump `EXPECTED_FAIL.md`** once (header + a Plan C section).
- [ ] **Step 4: Update the QUICK_REF std-module inventory + `MANIFEST.txt`** (every module + md5).
- [ ] **Step 5: Rotate the seed** if the distribution changed (the `lib/` payload grows even though the fixed point does not move):
...
- [ ] **Step 6: Record the program-completion pointer.**
...
- [ ] **Step 7: Commit** (`chore(std-lib): Plan C closeout — L4 + L5 landed; program complete`).

---

## Self-Review

- **Spec coverage:** spec §4 Plan C (all nine modules) → Tasks 1-8; §5 R1/R3/R6 → the constraints; §6 crypto gate → Task 1 Step 5; §7 distribution → Task 9 Steps 1/4/5; §10 index → the `Sequence:` line + Task 9 Step 6.
