...
- [ ] **Step 0 (AMENDMENT 1, 2026-09-09): Pre-evaluate the Task-4 review findings before implementing.** Two Task-4-review findings carry into this task (operator ruling 2026-09-09: "delegate to Task 5 to evaluate"; no Task-4 code fix):
  - **R1 — Task-4 pass scope wider than "join-temp only".** `coalesceJoinCopies` (landed Task 4, `lir_opt_pass.zig`) admits ANY single-def/single-use/cross-block `name_id==0` copy, not only genuine multi-arm merge temps (no `wc[dst]>=2` arm-count test) — the likely driver of Task-4's 1,580-vs-1,191 overage, partially pre-absorbing this task's inventory (storecons/exprcons/single-def branchret). **Evaluate, then EITHER** (a) add a `wc[dst]>=2` multi-arm shape test to restrict Task-4's pass to genuine joins AND implement the true remaining cases here under the gate, **OR** (b) accept the union scope (Task 4 already coalesced those shapes soundly), document it, and implement only the still-remaining coalescible cases here. Pick per evidence; if the pass is left unrestricted, re-scope this task's ledger arithmetic to the remaining inventory. Semantics are unaffected either way (verified by Task-4 review: gate is §4.1-verbatim on real LIR counters, no escape, run-identity 24/24 + corpus zero-asymmetric).
  - **R2 — param-assign dsts not flag-excluded (benign, record).** Scalar params register via `addLocalDecl` (no `decl_local` inst), so `ex[dpos]==0`; but analysis (operator-agreed 2026-09-09) shows unreachability: scalar param reads lower via `.load_local{name_id}` (never `markRead(param_temp)`), so `rc[param_temp]==0` and the `rc[dpos]==1` gate at `lir_opt_pass.zig:1504` skips them; aggregates fail `copyScalarKindOk`. Even if reached, the retarget is value-preserving (param temps registered in `fl_temps` at c89_emit.zig:2830-2855 → identical C render via `resolveTempName`). **Record benign; no guard required.** Optionally add an explicit name-mapped/fl_temps dst exclusion ONLY if the evaluation surfaces a reachable counter-example.
  - Record the R1 ruling + evidence in the report (`## Task 5`, Step 0 subsection) before implementing.
- [ ] **Step 1: Implement coalescing for every remaining coalescible case** Task 1 enumerated (as re-scoped by Step 0), each under the same gate. Cases the census tagged "keep" are NOT forced.
