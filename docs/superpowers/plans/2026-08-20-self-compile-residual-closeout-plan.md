...
## AMENDMENT 11 (operator ruling, 2026-08-21): F-A2EXT re-scoped — I-step for type-storage definition site (Option A)

**Context:** Task F-A2EXT (AMENDMENT 8 Ruling 2) was dispatched with the mechanism "the `emitModuleHeaderFile` type-storage branch must emit the `extern` in **every** header (drop the owner-module filter)". The implementer returned **BLOCKED** with verified evidence that the mechanism is wrong (report `.superpowers/sdd/task-FA2EXT-report.md`, controller re-verified):

1. **The 9 residual `zG_` symbols are NOT in `global_decls` at all.** Instrumentation showed 110 entries (59 type-storage); none match `SymbolKind`/`CoercionKind`/`LirInst`. So no `emitModuleHeaderFile` breadth change can reach them.
2. **"Extern in every header" is unsound C** where the referenced type is not declared in that TU (fixture `emission_type_storage_extern_xmod` broke with `unknown type name` in std headers; it was already GREEN at HEAD via the owner filter).
3. **Root cause (controller-verified):** the 3 broken types are enums/unions (`SymbolKind` in `symbol_table.zig:14`, `CoercionKind` in `coercion.zig:1`, `LirInst` in `lir.zig:22`) that are **only ever aliased import-base** (`const SymbolKind = @import("symbol_table.zig").SymbolKind;` — e.g. `analyzer.zig:15`, `lower.zig:2/:21`, `c89_emit.zig:7`), never ident-base (`const X = mod.X`). The storage-kill at `main.zig:603-606` (`field_access` with `import_expr` base → `gv_is_storage = 0`, added in `1ab2abdb` F-7) suppresses import-base aliases to avoid duplicate defs across many importing modules — but it **over-suppresses**: it also kills the *one* def that should exist in the type's owner module. The working `AstKind` def (`zT_8143F551_AstKind zG_8143F551_AstKind;` in `ast_2FA12982.c:2`) exists only because `main.zig:46` aliases it **ident-base**. `enum_decl`/`union_decl` themselves never create a storage global (per `02_symbol_registration.md:107-115`); only ident-base `var_decl` aliases do.

**Operator ruling:** proceed with **Option A** — define type-storage globals from the *type declaration* in the type's *owner module*, not from aliases. A new **I step** (read-only) must pin the exact mechanism before any F edit.

### I step (NEW, read-only, executes before F-A2EXT's F edit)

In `sf/src/`, investigate and report the exact design for emitting a type-storage global definition in the type's owner module for every named type (enum/union/struct, and any other value-position type) that is referenced as a runtime value anywhere in the module graph. Specifics:

1. **Inventory the type-storage-global producers today.** Confirm the ONLY producer is the module-scope `var_decl` storage-global registration (`main.zig:585-629`, `gv_is_storage` + the `import_expr`/`field_access` storage-kill at `:603-606`), and that `enum_decl`/`union_decl`/`struct_decl` never register a storage global themselves (`symbol_registrator.zig` struct/enum/union branch `:334-...` creates `SymbolKind.type_alias` only). Cite lines.
2. **Design the owner-module definition.** For each named type T declared in module M (via `typeRegistryRegisterNamedType`, `types_items[T].module_id == M`), decide when M must emit `zT_<hash>_<T> zG_<hash>_<T>;` — i.e. when T is referenced in value position (enum-member access, `load_global`/`store_global` sites `lower.zig:1391/:2393`, `@enumToInt`, switch-on-tag, `.tag`/`.payload` access). Determine the *reference signal* the emitter can use to know a type needs its storage global (e.g. a global-decls-like scan, a set of "types referenced as values", or reuse of the type dependency graph). State the exact function(s) in `c89_emit.zig` (`emitGlobalDecls`, `emitModuleHeaderFile`, `emitSpecialTypes`/`zig_special_types.h` path) where the def and extern would be emitted, and how `nameManglerMangleGlobal`'s type-identity keying (`(ty.name_id, ty.module_id)`) already guarantees the def/extern/refs agree.
3. **Extern placement.** Where the extern for a type-storage global must go so every referencing module sees it WITHOUT the unsound "every header" approach: candidate = owner module's header (for types whose owner has a real header) and/or `zig_special_types.h` (for the 3 special types owned by the pseudo-module). Confirm the include-chain property (each referencing module's include chain reaches the declaring header) holds for the 9 residuals.
4. **Blast radius / byte-identity.** Which current programs would change: the 9 residuals (SymbolKind/CoercionKind/LirInst — currently gcc-error, so adding defs cannot regress a working gate); confirm json `g_arena` (a *value* global, `*Arena` type, name_id 0 → not type-storage) stays per-module storage-killed; confirm the ident-base case (`AstKind`) still emits exactly one def (no duplicate).
5. **Fixture confirmation.** Under the chosen design, `emission_type_storage_extern_xmod` + `emission_mangler_collision_xmod` stay GREEN, and the 9 `zG_` self-compile errors go to 0.
