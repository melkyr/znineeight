...
- Produces: `pool.peak` drop toward the headroom prize (expected ~2–3 MB with the 1 MiB cap; the headroom class is bounded by the canonical geometric-growth reality, not the full 5.5 MB).

- [ ] **Step 1: Golden baseline capture**

Capture `/tmp/golden_F2/` (same set as F-1 Step 1) + record baseline `pool=`.

- [ ] **Step 2: Pin the tuning mechanism (operator-ruled AMENDMENT 4, 2026-09-01)**

**Policy: lower the growth cap from 2 MiB to 1 MiB; keep 2× doubling + exact-fit-final unchanged.** Rationale (operator + canonical LLVM-style slab-cap): the 2× geometric growth is already the standard amortized policy and MUST NOT be abandoned (no need-based/1.5× rewrite — that would trade headroom for worse total carve + F-1-style ladder distortion). The 2 MiB cap is the sole "sane slab size" knob: lir_read's 1.37 MB function then exact-fits to ~1,367,848 B via the existing `size > new_size` rule instead of carving a near-empty 2 MiB slab (the single largest headroom offender, 2,096,720 B), and perm/module/scratch/type_db cap segments shrink likewise. In `growableSandGrow` change only `max_segment` 2 MiB → 1 MiB (keep `new_size = last.size * 2`, cap at 1 MiB, then `if (size > new_size) new_size = size`). If the measured `pool=` drop is smaller than ~2 MB and the operator approves a second variant, try 512 K — otherwise stop at 1 MiB. Do NOT over-engineer (operator directive): no growth-policy rewrite, no need-based growth, no other tuning.
...
Report the `pool=` delta (expected: ~2–3 MB with the 1 MiB cap).
