...
**AMENDMENT 17 (operator-ruled 2026-08-26, S-INTERNER CLOSED UNFEASIBLE):** the S-INTERNER Step 1 census (report `## S-INTERNER-I`) found the write-through infeasible: emission = **95.9% of Gets (838,798 reads / 7.36 MB re-read)**; **63% of text is interned DURING emission** (concurrent write/read, not quiescent); **total text bulk = 288 KiB, NOT the ~4 MB the AMENDMENT-11 estimate assumed (14× over)**; and **RETAINED slices exist** across the emitter (`emitFieldAssign` c89_emit.zig:277-352, `emitTaggedUnionType` :1465, `emitSliceType` :1853, `emitArrayType` :1657, fn-signature :1968-2040, `emitGlobalDecls` :2445-2467, statement emitters :5414-5450/:6144-6332/:6883-6943) plus Get-result-passed-as-Intern-arg (type_resolver.zig:699, const_alias_prepass.zig:134/209, c89_emit:1948). Per-name fseek+fread = prohibitive (839 K disk reads); batch-fault = self-defeating (text written during emission); fault-in-on-Get with transient buffers = infeasible without rewriting the whole emitter (only mmap-style stable backing would satisfy retention — unavailable on Win9x). Pool prize ≈ 0.29 MiB (−1.1%) at highest risk. **Operator ruling: keep interner text resident; S-INTERNER Step 3 (F) NOT executed; task CLOSED unfeasible** (M5 precedent). Real levers already landed: S-LIR/S-HASH/S-TOKEN/S-AST/S-RES. Proceed to the S-FIX series, then I-COMPACT, then GATE. Expected floor ≈ 25.5 MiB pool (target 16,384 K; residual gap ~9.4 MiB documented).

**AMENDMENT 18 (operator-ruled 2026-08-26, I-COMPACT CLOSED NO-GO + final residual):** the I-COMPACT evaluation (report `## I-COMPACT`) found the 16-B AstNode is a **byte-identical shuffle, not a saving**: current 24 B node (span+child_2 inline) + 4 B payload = 28 B/node; "16-B" moves span/child_2 out-of-line = the same 28 B. Spans disk-parallel → record 131,072 B (worse than 114,688, window +128 KiB); spans resident → +2.2 MiB pool ADD. Honest pool.peak effect = 0 or negative — the pre-streaming "−2.9 MB" is invalidated (the pool-resident AST is only the 8-slot window ≈ 0.88 MiB). Migration cost re-verified: **47** AstNode span-read sites (plan's ~700 overcounted — ~400 are token spans) + 45 child_2 sites. Measured post-S-series: `pool=25742K` (total=5755K), **gap to 16,384K = 9,358K ≈ 9.14 MiB**; the S-series + S-FIX delivered pool 50,501K → 25,742K (−24,759K ≈ 48%). **Operator ruling: NO-GO accepted; I-COMPACT CLOSED with the evaluation as its deliverable** (revisit only if the AST ever goes resident again). The ≤16,384K target is NOT reached; the documented residual (pool=25,742K) is the S-series outcome, recorded at GATE per the plan's gate line.
...
### Task S-INTERNER: Write-through interner text — CLOSED UNFEASIBLE (AMENDMENT 17)

> **AMENDMENT 17 (operator-ruled 2026-08-26):** the Step 1 census (`## S-INTERNER-I`) showed the write-through infeasible and the prize negligible: emission = 95.9% of Gets (838,798 reads / 7.36 MB re-read); 63% of text interned during emission (concurrent write/read); total text bulk = **288 KiB, not ~4 MB** (14× over-estimate); **RETAINED slices exist** across the emitter → transient-buffer fault-in breaks them (only mmap-style stable backing would work — unavailable on Win9x); per-name fseek+fread prohibitive (839 K reads); batch-fault self-defeating. Pool prize ≈ 0.29 MiB at highest risk. **Operator ruled: keep interner text resident; Step 3 (F) NOT executed; task CLOSED unfeasible** (M5 precedent). Real levers already landed (S-LIR/S-HASH/S-TOKEN/S-AST/S-RES).
...
- [ ] **Step 1: (I) census** — DONE (report `## S-INTERNER-I`): read cost prohibitive, RETAINED slices, prize ~0.29 MiB → Step 3 F NOT executed (operator ruling, AMENDMENT 17). Steps 2/4/5 superseded by the closure.
...
### Task I-COMPACT: clever 16-B AstNode compaction evaluation — CLOSED NO-GO (AMENDMENT 18)

> **AMENDMENT 6 + 11 + 18:** the evaluation (report `## I-COMPACT`) concluded NO-GO: the 16-B layout is a byte-identical shuffle (28 B/node either way); pool.peak effect 0 or negative under the block-backed design; migration cost 47 span-read + 45 child_2 sites; gap 9,358K not closed. **Operator ruled NO-GO accepted (m1112); task CLOSED with the evaluation as its deliverable.** Revisit only if the AST goes resident again.
