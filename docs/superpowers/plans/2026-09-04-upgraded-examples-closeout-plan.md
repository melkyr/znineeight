...
- Every canonical feed run must remain **byte-identical between the original program and its `_upgraded`** counterpart. Demo goldens (new surface) are authored at GREEN time and verified 3×; exception (AMENDMENT 4, operator ruling): the lisp demo's `(address ...)` line prints a runtime pointer and is therefore NOT byte-deterministic across runs under the default PIE/ASLR build — this is documented in the example README and verified behaviorally (no determinism flags are added).
...
Verify each `(define …)`/quote form parses under the upgraded REPL (the interpreter supports define/quote per the error vocabulary; substitute an equivalent expression if any form errors, noting it in the report). `allocs` is a deterministic count. `address` prints a runtime pointer value (AMENDMENT 4) — see Step 4 for how its determinism caveat is handled.

- [ ] **Step 4: Capture + verify the demo golden.** Build, run the feed 3×. Determinism requirement (AMENDMENT 4): ALL output lines EXCEPT the `(address ...)` integer line must be byte-identical across the 3 runs; the `(address ...)` line prints a runtime pointer VA that varies run-to-run under the default build (gcc PIE + kernel ASLR — `et_dyn`, `randomize_va_space=2`) — do NOT add `-no-pie` or any determinism flag, do NOT treat the variance as a failure; record the three observed VAs in the report as the evidence of the documented behavior. Cross-check the deterministic values against hand contracts: `layout` line prints `8 1 64` (`EnvNode.value` @ byte 8; `@bitSizeOf(bool)`=1; `@bitSizeOf(i64)`=64); `bitcast` prints `-1`; `container-of` prints `true`; `ptr-check` prints `true` (this is the real @ptrFromInt round-trip validation — stable across runs); `classify` prints `2 2 2` (AMENDMENT 3: the lisp reader has no string-literal type, so the symbol is quote-wrapped `ab12XY` = 2 lower + 2 digit + 2 upper); `eq?` atom lines `true/false/true/false`. If any deterministic line contradicts its hand contract, STOP-present (do not silently re-baseline). Write run-1's bytes to `demo_expected.txt` as a reference capture (the REPL prints `> ` prompts and `nil` for Nil-returning builtins — these bytes are part of the golden, captured, not assumed); the README (Step 6) documents that this file's `(address)` integer is a per-run sample, not a byte gate.
...
- [ ] **Step 6: Demo README.** `demo/README.md`: the 8 builtins, the two feeds, and the note that `demo_expected.txt` includes a runtime pointer sample (implementation-coupled; regenerate if perm-arena allocation order ever changes). The README MUST carry the determinism note (AMENDMENT 4, operator-directed): the demo output is NOT byte-deterministic end-to-end — `(address ...)` prints the runtime address of a perm-arena `Value`, and because the build links as PIE by default while the kernel applies ASLR, that printed integer varies on every run (explain: gcc default `-pie`/ET_DYN + `randomize_va_space=2` relocates the static arena base per process). The committed `demo_expected.txt` is a reference capture whose `(address)` line is a per-run sample; behavior must be verified (the other ten deterministic lines byte-match the contracts, `(address)` prints a positive integer, and `(ptr-check ...)` returns `true` proving the @ptrFromInt round-trip), not by md5 of the whole file.
