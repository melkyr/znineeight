...
- [ ] **Step 7b — build_test.sh link (OPERATOR RULING 2026-08-01):** Append `"$ROOT_DIR/src/include/zig_pal.c"` to the gcc link line in `sf/scripts/build_test.sh` (after `$c_files`). Without this, all 9 test binaries FAIL to link (undefined `pal_file_*` from emitted `pal.c`). Update any doc recipe referencing build_test.sh (QUICK_REF "Build and Run Tests", AGENTS.md §9.2) to note the link. **Gate (OPERATOR RULING 2026-08-01):** result is `5 passed / 4 failed`, **identical to base `44ccc246`** (pre-existing failures: test_semantic_bin/test_sym_reg_bin at zig0, test_mod_reg_bin/test_analyzer_integration_bin at run — outside F-S1 scope, non-regressing). F-S1 is DONE at 5/4; the 4 pre-existing failures are NOT fixed in F-S1.
