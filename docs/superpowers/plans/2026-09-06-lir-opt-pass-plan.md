...
**Backend-agnostic boundary (binding):** the pass never reasons about C89 emission shape. Per-temp facts computed generically: `inline_candidate` (produced by a PURE inst AND single value use AND not address-taken AND not excluded [param/decl_local-bound/load_global-alias] AND **rule (d): consumer same-block with no ORDERED inst strictly between def and consumer** [operator ruling 2026-09-08, T4b review finding #1 — a memory-reading PURE def nested across an ORDERED window would observe aliased writes twice/stale; register-only nesting is unaffected today, but the candidate flag must be sound for ALL pure defs so any backend can trust it]) + generic expression-tree `depth` (unbounded; C90 cap is the emitter's concern). Stored as a per-function sand side-table (option A). Emitter reads candidate+flag+depth, then applies rule (h)/(i) materialization guard + the three re-expressions (INTWIDTH width-wrap, is_checked int_cast, signed-narrow load_bitfield block) + depth-cap fallback. `lir.zig` / `lir_stream` format untouched. Memory: per-function tables sized to one function's temp count (existing Task-3 pattern — 13 per-temp arrays from Sand) → no spill needed, nothing accumulates across functions (sandReset).
...
- [ ] **T4b-FIX (rule (d) enforcement, operator ruling 2026-09-08):** pass-side, backend-agnostic, ZERO emission change. Extend the `inline_candidate` predicate in `lir_opt_pass.zig` with rule (d): a def's single consumer must be in the SAME block as the def AND no ORDERED (side-effecting) LirInst may sit strictly between the def and the consumer in that block (one forward sweep per block tracking the last-ORDERED index; O(block + candidates)). Because no memory-reading def is nested today (T4b nests register/const PURE shapes only), this changes ZERO emission — it makes the candidate flag sound-by-construction for all pure defs (memory-reading included) so the metadata any future backend or later increment consumes is trustworthy. Gate: golden 9/9 + matrix 21/21 run byte-identical (unchanged vs T4b); 4-MD5 unchanged (gol `f9225cfe`/lisp `2167075b`/json `af40da09`/mud `79cafa18` — the T4b POST values, NOT re-baselined); fixed point unchanged (no sf/src emission delta beyond self-source growth per Ruling A — record the N-hop value). Commit: `fix: LIR opt pass — enforce rule (d) same-block write-free window in nest candidate (T4b-FIX LIROPTPASS)`.
