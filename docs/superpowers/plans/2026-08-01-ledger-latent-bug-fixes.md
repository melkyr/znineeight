...
- [ ] **Step 2 — Fix tstIsDep (REQUIRED):** Add missing source-kind branches after error_union branch (:851-852): `optional_type` → `reg.opt_items[payload_idx].payload`, `slice_type` → `reg.slice_items[payload_idx].elem`, `union_type` → iterate `reg.un_items[payload_idx].fields_start/count` over `fe_items`. Exact code in IM8 §D2.
- [ ] **Step 3 — Fix tstEdgesCount + tstEdgesFill (REQUIRED):** Mirror the optional/slice/union source branches (IM8 §D3/D4). No enum/error_set SOURCE branches needed (backing_type/tags are plain ints). tstEdgesFill is dead code but fix for completeness (AMENDMENT 2).
...
- [ ] **Step 5 — Gate:** `bash sf/scripts/build_release.sh` → 0 errors. Run IM8 §A5 repros #1/#2/#3 → gcc clean (no `unknown type name`). Bare `--dump-c89` md5s preserved (assessment verified no struct in the 4 gated examples has only-enum fields → classification unchanged → stdout stable). Lisp `--output-dir` partition still correct. Corpus: zero regressions.
