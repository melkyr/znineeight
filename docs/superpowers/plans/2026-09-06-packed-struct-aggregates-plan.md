...
- [ ] **Step 3: Chain analyze.** Per I-1 Step 4: `lowerPackedChainAnalyze` treats the packed-union container step correctly; verify read + store land as ONE accumulated-offset `load_bitfield`/`store_bitfield`. **AMENDMENT ruling A (operator 2026-09-08):** I-1 found the plan's "whole-member already rejected, no extra code" assumption WRONG for the direct depth-1 case — `u.member` (whole packed-struct member of a packed union, no `.field` after it) reaches the generic packed_union read `lower.zig:3088` / store `:1431`, which lack the whole-sub type guard (the packed-*struct* twins at `:3075`/`:1387` have it). So F-1 MUST add 2 whole-member guards to those union read/store paths (leaf-typed packed-struct → clean `error[3000]`, rc=2, 0 `.c`, never silent). This is IN scope per the operator ruling — not a "do not add" gap. **AMENDMENT ruling A2 (operator 2026-09-08):** F-1 review found a THIRD whole-member route still silent — the packed-union AGGREGATE-LITERAL store `U{ .b = someInner }` (union-literal initializer whose value is a whole packed-struct) at `lower.zig` ~:4279-4288 emits `store_bitfield` with the inner `total_bits` and no member-type guard → struct-typed bitfield op → silent GCCFAIL. Per the same never-silent ruling (4), F-1 MUST add a member-type guard to that agg-literal route so `U{ .b = someInner }` = one clean `error[3000]`, rc=2, 0 `.c`.
