...
`intwidth_full_xmod/main.zig` — contract `0 -1` (u64 boundary wrap via alias-width u64 == existing u64; i64 -1 sign-extends at full width):
```zig
const std = @import("std");

pub fn main() void {
    var a: u64 = @intCast(u64, 18446744073709551615);
    var b: u64 = @intCast(u64, 1);
    var s = a + b;
    std.io.printInt(@intCast(i64, s));
    std.io.writeByte('\n');
    var n: i64 = @intCast(i64, -1);
    std.io.printInt(n);
    std.io.writeByte('\n');
}
```
