// stdlib_bits_extract_trap_xmod — STDLIB std_bits (L0) extract out-of-range
// expected-failure probe (Plan A hardening Task 3).
//
// Contract (sf/src/std_bits.zig:112-123): extract traps via `unreachable` when
...
pub fn main(argc: i32, argv: [*]*const u8) void {
    ...
    var len: u32 = 8;
    var r = bits.extract(@intCast(u32, 0), off, len);
    _ = r;
    std.io.write("bits extract no-trap FAIL\n");
}
