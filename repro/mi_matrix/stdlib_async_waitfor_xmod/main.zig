// stdlib_async_waitfor_xmod — Track 4c std.async.waitFor fixture (main context).
//
// `waitFor(&s, &s.tasks[0])` is called from `main` (a non-suspending context):
// it drives the scheduler's tick loop until t0 is settled, with no caller frame
// and no `@asyncSuspend`. t0 yields twice, then completes with `val = id*10`.
//
