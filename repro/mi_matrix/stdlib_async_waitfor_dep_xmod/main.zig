// stdlib_async_waitfor_dep_xmod — Track 4c std.async.waitFor control (Q4: chains).
//
// t1's coroutine calls `awaitTask(&s, &s.tasks[0])` (t0), so t1 cannot settle
// until t0 does. `waitFor(&s, &t1)` from main must therefore tick the whole
// dependency chain to completion: t0 first (val 10), then t1 (val 20).
//
...
//   stdout `10` then `20` then `3` (t0.val, t1.val, t1 TaskState.done).
...
pub fn main() void {
    ...
    sa.waitFor(&s, &t1) catch p(-1);
    p(f0.val);
    p(f1.val);
    p(@intCast(i32, @intCast(u8, t1.state)));
}
