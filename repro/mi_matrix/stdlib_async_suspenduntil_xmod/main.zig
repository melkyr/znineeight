...
//
// The driver runs a single coroutine that calls `sa.suspendUntil(isReady)`.
...
pub fn main() void {
    ...
    if (task.state != sa.TaskState.suspended) @panic("expected suspended");
    ...
    if (task.state != sa.TaskState.done) @panic("not done");
