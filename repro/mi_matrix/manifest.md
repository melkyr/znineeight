...
|-----|------|------|--------|-------------|
| v12 | 0f750ef6 | 2026-07-17 | **168 dirs, OK=164, FAIL=4, ICE=0, CRASH=0** | json_parser runtime bug fixes (4 commits). Fixed: (1) block_terminated leak → parserSkipWhitespace single-char skip (f9ee0b7f); (2) field-store-through-slice-index value-copy (0c959169); (3) scope-based variable tracking — local_decl_scopes array, scope-filtered findLocalTemp, captures at scope_depth+1, reverted Fix C overwrite, kept resolveTempName (0f750ef6). json_parser compiles 0 gcc errors, parses JSON correctly (3 pre-existing output diffs: version number, comma formatting). Lisp (+ 1 2)=3 no regression. Excluded c168e610 (broken Fix B+C). Known FAILs unchanged. |
