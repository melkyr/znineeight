...
// `%` is UB. Under `-fsafe` (default) the div/mod guard traps before the
// operation (empty stdout, rc 133 SIGTRAP); under `-ffast` no guard is emitted:
// 32-bit gcc lowers the `i64` modulo to the `__moddi3` runtime helper, which does
// not raise SIGFPE, so the process exits rc 0 (printing `0`) — still UB, just
// not a hardware trap.
//
