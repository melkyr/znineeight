...
pub fn readFile(arena: *void, path: []const u8) FileError![]u8 {
    // We need to null-terminate the path for C fopen
    // But for simplicity in this example, let's assume it's already null-terminated or use a buffer
    var path_buf: [256]u8 = undefined;
    var i: usize = 0;
    while (i < path.len and i < 255) {
        path_buf[i] = path[i];
        i += 1;
    }
    path_buf[i] = 0;

    const mode: [*]const c_char = @ptrCast([*]const c_char, "rb");
    const f = fopen(@ptrCast([*]const c_char, &path_buf[0]), mode) orelse return error.OpenFailed;
    defer _ = fclose(f);
