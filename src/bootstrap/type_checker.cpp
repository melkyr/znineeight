...
Type* TypeChecker::visitBlockStmt(ASTBlockStmtNode* node) {
    ...
    if (node->statements) {
        for (i = 0; i < node->statements->length(); ++i) {
            ...
            if (last_type && is_type_undefined(last_type)) {
...
Type* TypeChecker::visitBreakStmt(ASTNode* node) {
    ...
    if (in_defer_) {
        reportAndReturnUndefined(node->loc, ERR_BREAK_INSIDE_DEFER, NULL);
    } else if (current_loop_depth_ == 0) {
        reportAndReturnUndefined(node->loc, ERR_BREAK_OUTSIDE_LOOP, NULL);
    }

    if (!resolveLabel(break_node.label, break_node.target_label_id)) {
        if (break_node.label) {
            reportAndReturnUndefined(node->loc, ERR_UNKNOWN_LABEL, NULL);
        }
    }
...
Type* TypeChecker::visitContinueStmt(ASTNode* node) {
    ...
    if (in_defer_) {
        reportAndReturnUndefined(node->loc, ERR_CONTINUE_INSIDE_DEFER, NULL);
    } else if (current_loop_depth_ == 0) {
        reportAndReturnUndefined(node->loc, ERR_CONTINUE_OUTSIDE_LOOP, NULL);
    }
...
Type* TypeChecker::visitReturnStmt(ASTNode* parent, ASTReturnStmtNode* node) {
    if (in_defer_) {
        reportAndReturnUndefined(node->expression ? node->expression->loc : parent->loc, ERR_RETURN_INSIDE_DEFER, NULL);
    }
    ...
    if (current_fn_return_type_->kind == TYPE_VOID) {
        if (node->expression) {
            ...
            reportAndReturnUndefined(node->expression->loc, ERR_INVALID_RETURN_VALUE_IN_VOID_FUNCTION, NULL);
        }
    }
    /* Case 2: Function is non-void */
        ...
        if (!node->expression) {
            ...
            if (current_fn_return_type_->kind == TYPE_ERROR_UNION &&
                ...
                reportAndReturnUndefined(parent->loc, ERR_MISSING_RETURN_VALUE, NULL);
            }
        } else {
            // Implicit Array to Slice coercion
...
Type* TypeChecker::visitSwitchExpr(ASTSwitchExprNode* node) {
    ...
    for (i = 0; i < node->prongs->length(); ++i) {
        ...
        if (prong->capture_name) {
            if (!is_tagged_union) {
                reportAndReturnUndefined(node->expression->loc, ERR_TYPE_MISMATCH, "Capture only supported for tagged union switch");
            } else if (prong->is_else) {
                reportAndReturnUndefined(node->expression->loc, ERR_TYPE_MISMATCH, "Capture not supported for else prong");
            } else if (prong->items->length() != 1) {
                reportAndReturnUndefined(node->expression->loc, ERR_TYPE_MISMATCH, "Capture in switch prong only allowed with a single case");
            } else {
...
Type* TypeChecker::visitArrayType(ASTArrayTypeNode* node) {
    ...
    /* 1. Handle slices */
    if (!node->size) {
        if (!node->element_type) return NULL;
...
Type* TypeChecker::visitTryExpr(ASTNode* node) {
    ...
    if (!try_node.expression) return NULL;
...
Type* TypeChecker::visitFunctionType(ASTFunctionTypeNode* node) {
    ...
    if (node->params) {
        for (i = 0; i < node->params->length(); ++i) {
            param_type = visit((*node->params)[i]);
            if (param_type) {
    ...
    if (!node->return_type) return NULL;
    return_type = visit(node->return_type);
