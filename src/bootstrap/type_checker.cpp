...
Type* TypeChecker::visit(ASTNode* node) {
    ...
    /* SOFT Registry Consistency Check */
    const char* type_name = NULL;
    Module* expected_owner = unit_.getModule(unit_.getCurrentModule());
    if (node->type == NODE_STRUCT_DECL) type_name = node->as.struct_decl->fields && node->as.struct_decl->fields->length() > 0 ? "dummy" : NULL; 
    else if (node->type == NODE_UNION_DECL) type_name = node->as.union_decl->fields && node->as.union_decl->fields->length() > 0 ? "dummy" : NULL;
    else if (node->type == NODE_ENUM_DECL) type_name = current_struct_name_;
    else if (node->type == NODE_ERROR_SET_DEFINITION) type_name = node->as.error_set_decl->name;
    ...
    switch (node->type) {
        ...
        case NODE_STRUCT_INITIALIZER: resolved_type = visitStructInitializer(node, node->as.struct_initializer); break;
        case NODE_TUPLE_LITERAL:    resolved_type = visitTupleLiteral(node, node->as.tuple_literal); break;
    ...
    if (resolved_type && !is_type_undefined(resolved_type)) {
        /* Softened Registry Check Implementation */
...
Type* TypeChecker::visitCompoundAssignment(ASTCompoundAssignmentNode* node) {
    ...
    if (!IsTypeAssignableTo(result_type, lvalue_type, node->lvalue->loc, node->rvalue)) {
        /* IsTypeAssignableTo already reports a detailed error. */
        return get_g_type_undefined();
    }
...
bool TypeChecker::needsStringLiteralCoercion(ASTNode* src, Type* target) {
...
void TypeChecker::coerceStringLiteralToSlice(ASTNode** expr_ptr, Type* target_type, SourceLocation loc) {
    ASTNode* literal;
    size_t len;
    ASTNode* slice_node;
    ASTArraySliceNode* slice_data;

    literal = *expr_ptr;
    len = plat_strlen(literal->as.string_literal.value);
    ...
    slice_data = (ASTArraySliceNode*)unit_.getArena().alloc(sizeof(ASTArraySliceNode));
    plat_memset(slice_data, 0, sizeof(ASTArraySliceNode));
    slice_data->array = literal;

    /* Manually populate for string literal to avoid visitArraySlice rejecting the single-item pointer. */
    slice_data->base_ptr = literal;
    slice_data->len = createIntegerLiteral(len, get_g_type_usize(), loc);

    slice_node->as.array_slice = slice_data;
...
Type* TypeChecker::visitIdentifier(ASTNode* node) {
    ...
    if (sym->symbol_type && sym->symbol_type->kind == TYPE_TYPE && sym->details && sym->kind == SYMBOL_VARIABLE) {
        ...
        if (decl->initializer && decl->initializer->resolved_type && decl->initializer->resolved_type != get_g_type_type()) {
...
Type* TypeChecker::resolvePlaceholder(Type* placeholder) {
    ...
    if (resolved->kind == TYPE_TYPE) {
        if (placeholder->as.placeholder.decl_node && placeholder->as.placeholder.decl_node->type == NODE_VAR_DECL) {
    ...
    if (resolved != placeholder && resolved->kind != TYPE_PLACEHOLDER) {
        finalizePlaceholder(placeholder, resolved);
    } else if (resolved == placeholder) {
        /* Already finalized by nested visit call? Ensure names and layout are refreshed. */
        if (placeholder->kind != TYPE_PLACEHOLDER) {
             finalizePlaceholder(placeholder, placeholder);
        }
    }
#ifdef DEBUG
    if (placeholder->kind == TYPE_PLACEHOLDER) {
...
bool TypeChecker::validateRange(ASTRangeNode* range, Type* cond_type) {
    ...
    if (start_val > effective_end) {
        unit_.getErrorHandler().report(ERR_INVALID_RANGE, range->start->loc, "Empty or inverted range (start > end)");
        return false;
    }

    const int MAX_RANGE_LABELS = 1000;
    i64 count = effective_end - start_val + 1;
    if (count > MAX_RANGE_LABELS) {
        char msg[128];
        char count_str[32];
        plat_i64_to_string(count, count_str, sizeof(count_str));
        plat_strcpy(msg, "Range too large (");
        plat_strcat(msg, count_str);
        plat_strcat(msg, " values > max 1000)");
        unit_.getErrorHandler().report(ERR_RANGE_TOO_LARGE, range->start->loc, msg);
        return false;
    }
...
bool TypeChecker::validateSwitch(ASTNode* cond, DynamicArray<ASTSwitchProngNode*>* prongs, bool is_expr, Type*& result_type, SourceLocation loc, Type* expected_type) {
    ...
    if (is_expr && !has_else) {
        unit_.getErrorHandler().report(ERR_TYPE_MISMATCH, cond->loc, "Switch expression must have an 'else' prong");
        return false;
    }
    ...
    if (is_expr) {
        if (!has_non_noreturn) {
            ...
            if (common_type->kind == TYPE_SLICE) {
                for (size_t i = 0; i < prongs->length(); ++i) {
                    coerceNode(&(*prongs)[i]->body, common_type);
                }
            }
        }
    } else {
        result_type = get_g_type_void();
    }
    ...
    if (node->initializer && evaluateConstantExpression(node->initializer, &const_val)) {
        ...
        if (declared_type) {
            if (isNumericType(declared_type)) {
                ...
                if (!IsTypeAssignableTo(init_t, declared_type, node->initializer->loc, node->initializer)) {
                    return get_g_type_undefined();
                }

                if (needsStringLiteralCoercion(node->initializer, declared_type)) {
                    coerceStringLiteralToSlice(&node->initializer, declared_type, node->initializer->loc);
                    init_t = declared_type;
                }
        ...
        if (node->initializer->type == NODE_STRUCT_INITIALIZER && !node->initializer->as.struct_initializer->type_expr) {
            ...
            if (declared_type && needsStringLiteralCoercion(node->initializer, declared_type)) {
                coerceStringLiteralToSlice(&node->initializer, declared_type, node->initializer->loc);
                initializer_type = declared_type;
            }
        }
        ...
        if (placeholder && initializer_type && !is_type_undefined(initializer_type) && initializer_type != placeholder) {
            finalizePlaceholder(placeholder, initializer_type);
...
Type* TypeChecker::visitFnSignature(ASTFnDeclNode* node) {
    ...
    for (i = 0; i < node->params->length(); ++i) {
        ...
        Type* param_type = unwrapType(param_node.type);
    ...
    return_type = unwrapType(node->return_type);
...
Type* TypeChecker::visitFnBody(ASTFnDeclNode* node) {
    ...
    plat_printf_debug("[TYPE] FnBody visiting VarDecl '%s'\n", stmt->as.var_decl->name);
        ...
        if (current_fn_return_type_->kind != TYPE_VOID) {
            ...
            bool is_error_void = (current_fn_return_type_->kind == TYPE_ERROR_UNION &&
                                 current_fn_return_type_->as.error_union.payload->kind == TYPE_VOID);

            if (!is_error_void && !all_paths_return(node->body)) {
...
Type* TypeChecker::visitUnionDecl(ASTNode* parent, ASTUnionDeclNode* node) {
    ...
    if (node->is_tagged) {
        if (node->tag_type_expr && node->tag_type_expr->type == NODE_IDENTIFIER &&
            ...
            tag_type = unwrapType(node->tag_type_expr);
    ...
    for (i = 0; i < node->fields->length(); ++i) {
        ...
        Type* field_type = unwrapType(field_node->type);
        if (field_type && field_type->kind == TYPE_PLACEHOLDER) {
            field_type = resolvePlaceholder(field_type);
        }
    ...
    if (base_type->kind == TYPE_MODULE || base_type->kind == TYPE_ANYTYPE) {
        ...
        if (target_mod) {
            ...
            if (registered) {
                ...
                node->symbol = NULL; // It's a type, not a symbol
                return get_g_type_type();
            }
...
bool TypeChecker::checkStructInitializerFields(ASTStructInitializerNode* node, Type* struct_type, SourceLocation loc) {
    ...
    for (size_t i = 0; i < node->fields->length(); ++i) {
        ...
        Type* val_type;
        {
            ExpectedTypeGuard guard(*this, field_type);
            val_type = visit(init->value);
        }
        ...
        if (val_type && is_type_undefined(val_type)) {
             /* For anonymous initializers, we allow undefined at this stage
                because coerceNode (called above) might have returned TYPE_ANONYMOUS_INIT
                or still be in a state of partial resolution. */
             if (init->value->type != NODE_STRUCT_INITIALIZER &&
                 init->value->type != NODE_TUPLE_LITERAL &&
...
Type* TypeChecker::visitTupleLiteral(ASTNode* parent, ASTTupleLiteralNode* node) {
    ...
    /* If context is anytype/type, resolve to a concrete tuple OR anonymous tuple. */
    if (!expected || expected->kind == TYPE_ANYTYPE || expected->kind == TYPE_TYPE) {
        if (expected && expected->kind == TYPE_TYPE) {
            /* Resolve to concrete tuple for type definitions. */
            void* mem;
...
Type* TypeChecker::visitStructInitializer(ASTNode* parent, ASTStructInitializerNode* node) {
    ...
    if (node->type_expr) {
        struct_type = unwrapType(node->type_expr);
        ...
        if (expected->kind == TYPE_ARRAY) {
            ...
            for (size_t i = 0; i < node->fields->length(); ++i) {
                ...
                if (init->value) {
                    ExpectedTypeGuard guard(*this, element_type);
                    visit(init->value);
                    coerceNode(&init->value, element_type);
                }
            }
            return expected;
            ...
            if (checkStructInitializerFields(node, expected, parent->loc)) {
             ...
             if (!node->fields || node->fields->length() == 0) {
                  ...
                  /* Non-empty literal in TYPE_TYPE context should fall through to error
                     or be handled by named type resolution elsewhere.
                     For now, we don't want to assign TYPE_ANONYMOUS_INIT here if a concrete type is expected. */
                  return get_g_type_undefined();
             } else {
                  /* Non-empty literal without structural context: resolve elements and stay anonymous for now. */
                  for (size_t i = 0; i < node->fields->length(); ++i) {
...
Type* TypeChecker::visitEnumDecl(ASTEnumDeclNode* node) {
    ...
    if (node->backing_type) {
        backing_type = unwrapType(node->backing_type);
            ...
            if (sym->symbol_type) {
                if (sym->symbol_type->kind == TYPE_TYPE) {
                    ...
                    if (sym->details && ((ASTVarDeclNode*)sym->details)->initializer) {
                        ...
                        if (resolved_type && resolved_type->kind == TYPE_TYPE) {
                             /* Double-unwrapping for nested type constants if needed */
                             ASTVarDeclNode* vd = (ASTVarDeclNode*)sym->details;
                             if (vd->initializer->resolved_type && vd->initializer->resolved_type != get_g_type_type()) {
                                 resolved_type = vd->initializer->resolved_type;
                             }
                        }
                ...
                if (resolved_type && resolved_type->kind == TYPE_PLACEHOLDER) {
                    resolved_type = resolvePlaceholder(resolved_type);
                }
            }
        }
...
Type* TypeChecker::visitErrorUnionType(ASTErrorUnionTypeNode* node) {
    ...
    IndirectionGuard guard(*this);
    Type* payload = unwrapType(node->payload_type);
    if (!payload || is_type_undefined(payload)) return get_g_type_undefined();

    Type* error_set = NULL;
    if (node->error_set) {
        error_set = unwrapType(node->error_set);
...
Type* TypeChecker::visitErrorSetDefinition(ASTNode* node) {
    ...
    return createErrorSetType(unit_, unit_.getModule(unit_.getCurrentModule()), decl->name, decl->tags, decl->name == NULL, NULL, self_placeholder);
}
...
bool TypeChecker::all_paths_return(ASTNode* node) {
    ...
    switch (node->type) {
        ...
        case NODE_BLOCK_STMT: {
            ...
            /* Iterate backwards to skip trailing empty statements (semicolons) */
            for (int i = (int)statements->length() - 1; i >= 0; --i) {
                ASTNode* stmt = (*statements)[i];
                if (stmt->type == NODE_EMPTY_STMT) continue;
                return all_paths_return(stmt);
            }
            return false;
        }
        case NODE_IF_STMT: {
        ...
        case NODE_WHILE_STMT: {
            /* For bootstrap, we don't assume while loops always return even if while(true) */
            return false;
        }
...
->as.switch_expr;
            ...
            for (size_t i = 0; i < sw->prongs->length(); ++i) {
                ...
                if (prong->is_else) has_else = true;
                if (!all_paths_return(prong->body)) return false;
            }
            return has_else;
        }
...
bool TypeChecker::IsTypeAssignableTo( Type* source_type, Type* target_type, SourceLocation loc, ASTNode* source_node) {
    ...
    /* Task 9.7: Slice -> many-item pointer coercion */
    if (target_type->kind == TYPE_POINTER && target_type->as.pointer.is_many &&
        source_type->kind == TYPE_SLICE) {
        if (areTypesCompatible(target_type->as.pointer.base, source_type->as.slice.element_type)) {
            /* Const correctness */
    ...
    /* Task 9.7: Array -> many-item pointer coercion */
    if (target_type->kind == TYPE_POINTER && target_type->as.pointer.is_many &&
...
Type* TypeChecker::visitFunctionType(ASTFunctionTypeNode* node) {
    ...
    if (node->params) {
        for (i = 0; i < node->params->length(); ++i) {
            ...
            param_type = unwrapType(param_node);
...
static ASTNode* createAnonymousUnionInitializer(CompilationUnit& unit, const char* tag_name, Type* target_type, SourceLocation loc) {
    ASTNode* init_node = (ASTNode*)unit.getArena().alloc(sizeof(ASTNode));
    plat_memset(init_node, 0, sizeof(ASTNode));
    init_node->type = NODE_STRUCT_INITIALIZER;
    init_node->loc = loc;
    init_node->as.struct_initializer = (ASTStructInitializerNode*)unit.getArena().alloc(sizeof(ASTStructInitializerNode));
    ...
    init_node->as.struct_initializer->fields = new (fields_mem) DynamicArray<ASTNamedInitializer*>(unit.getArena());
...
void TypeChecker::coerceNode(ASTNode** node_slot, Type* target_type) {
        ...
        if (isTaggedUnion(target_type)) {
            ...
            if (payload_type) {
                if (payload_type->kind == TYPE_VOID) {
                    ...
                    ASTNode* init_node = createAnonymousUnionInitializer(unit_, node->as.integer_literal.original_name, target_type, node->loc);
                    *node_slot = init_node;
                } else {
                    char msg[256];
                    char* curr = msg;
                    size_t rem = sizeof(msg);
                    safe_append(curr, rem, "Tag '");
                    safe_append(curr, rem, node->as.integer_literal.original_name);
                    safe_append(curr, rem, "' requires a value (non-void payload)");
                ...
                safe_append(curr, rem, "Tag '");
                safe_append(curr, rem, node->as.integer_literal.original_name);
                safe_append(curr, rem, "' not found in tagged union");
                reportAndReturnUndefined(node->loc, ERR_TYPE_MISMATCH, msg);
            }
    ...
    if (node->type == NODE_MEMBER_ACCESS && node->as.member_access->base == NULL) {
        if (isTaggedUnion(target_type)) {
            Type* payload_type = findTaggedUnionPayload(target_type, node->as.member_access->field_name);
            if (payload_type) {
                if (payload_type->kind == TYPE_VOID) {
                    // Transform .Tag into .{ .Tag } (struct initializer with no value)
                    ASTNode* init_node = createAnonymousUnionInitializer(unit_, node->as.member_access->field_name, target_type, node->loc);
                    *node_slot = init_node;
                } else {
                    char msg[256];
                    char* curr = msg;
                    size_t rem = sizeof(msg);
                    safe_append(curr, rem, "Tag '");
                    safe_append(curr, rem, node->as.member_access->field_name);
                    safe_append(curr, rem, "' requires a value (non-void payload)");
                    reportAndReturnUndefined(node->loc, ERR_TYPE_MISMATCH, msg);
                }
            } else {
...
Type* TypeChecker::unwrapType(ASTNode* node) {
    ...
    if (t->kind == TYPE_TYPE) {
        if (node->type == NODE_IDENTIFIER) {
            ...
            if (!sym) sym = unit_.getSymbolTable().lookup(node->as.identifier.name);
            if (sym) t = resolveTypeConstant(sym);
        } else if (node->type == NODE_MEMBER_ACCESS) {
            if (node->as.member_access->symbol) {
                t = resolveTypeConstant(node->as.member_access->symbol);
            }
        }

        if (t->kind == TYPE_TYPE && node->resolved_type && node->resolved_type != get_g_type_type()) {
...
void TypeChecker::verifyTypeIdentity(Type* type, const char* expected_name, Module* expected_module, SourceLocation loc) {
    if (!type || !expected_name) return;
    ...
    switch (type->kind) {
        ...
        case TYPE_UNION:
        case TYPE_TAGGED_UNION:
            actual_name = type->as.struct_details.name;
            actual_module = type->owner_module;
            break;
        case TYPE_ENUM:
            actual_name = type->as.enum_details.name;
            actual_module = type->owner_module;
            break;
        default:
            return;  // Primitive types don't need verification
    }

    if (actual_name && expected_name && plat_strcmp(actual_name, expected_name) != 0) {
        char msg[512];
        plat_strcpy(msg, "Type identity mismatch: expected '");
        plat_strcat(msg, expected_name);
        plat_strcat(msg, "', got '");
        plat_strcat(msg, actual_name ? actual_name : "NULL");
        plat_strcat(msg, "'");
        unit_.getErrorHandler().report(ERR_INTERNAL_ERROR, loc, msg);
    }

    if (actual_module && expected_module && actual_module != expected_module) {
        char msg[512];
        plat_strcpy(msg, "Type module mismatch: expected '");
        plat_strcat(msg, expected_module->name);
        plat_strcat(msg, "', got '");
        plat_strcat(msg, actual_module->name ? actual_module->name : "NULL");
        plat_strcat(msg, "'");
        unit_.getErrorHandler().report(ERR_INTERNAL_ERROR, loc, msg);
    }
}

Type* TypeChecker::resolveNamedType(Module* defining_mod, const char* name, Symbol* sym) {
    if (!defining_mod || !name) {
        if (sym && sym->symbol_type) return sym->symbol_type;
        return NULL;
    }
    ...
    if (!registered) {
        ...
        if (!registered && sym && sym->symbol_type) {
            registered = sym->symbol_type;
        }
    ...
    if (sym && sym->symbol_type) {
        if (sym->symbol_type->kind == TYPE_PLACEHOLDER && registered->kind != TYPE_PLACEHOLDER) {
            sym->symbol_type = registered;
        } else if (sym->symbol_type != registered &&
                   sym->symbol_type->kind != TYPE_PLACEHOLDER &&
                   registered->kind != TYPE_PLACEHOLDER) {
            // Force sync - this is the key fix for type identity
            sym->symbol_type = registered;
        }
    }
