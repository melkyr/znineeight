...
bool CBackend::generateBuildBat(const char* output_dir) {
    ...
    const char* runtime_cmd = "cl.exe /nologo /W3 /I. /c zig_runtime.c\n";
    plat_write_file(f, runtime_cmd, plat_strlen(runtime_cmd));

    // Compile all modules
    for (size_t i = 0; i < generated_sources_.length(); ++i) {
        const char* cmd_part1 = "cl.exe /nologo /W3 /I. /c ";
    ...
    // Link everything
    const char* link_cmd_start = "link.exe /nologo /out:";
...
bool CBackend::generateMakefile(const char* output_dir) {
    ...
    // Compile zig_runtime.c
    const char* runtime_cmd = "gcc -std=c89 -m32 -pedantic -Wall -O2 -I. -c zig_runtime.c -o zig_runtime.o\n";
    plat_write_file(f, runtime_cmd, plat_strlen(runtime_cmd));
    ...
    for (size_t i = 0; i < generated_sources_.length(); ++i) {
        ...
        const char* cmd_part1 = "gcc -std=c89 -m32 -pedantic -Wall -O2 -I. -c ";
        plat_write_file(f, cmd_part1, plat_strlen(cmd_part1));
    ...
    // Link everything
    const char* link_cmd_start = "gcc -m32 -o ";
    plat_write_file(f, link_cmd_start, plat_strlen(link_cmd_start));
    plat_write_file(f, executable_name_, plat_strlen(executable_name_));
