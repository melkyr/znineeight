...
ASTNode* Parser::parsePostfixExpression() {
    ...
    while (true) {
        if (match(TOKEN_LPAREN)) {
            ...
            expr = new_expr_node;
        } else if (peek().type == TOKEN_LBRACE && (expr->type == NODE_IDENTIFIER || expr->type == NODE_TYPE_NAME || expr->type == NODE_MEMBER_ACCESS || expr->type == NODE_ARRAY_TYPE)) {
            // Struct or Array Initializer: Type { .field = value } or Type { val1, val2 }
...
ASTNode* Parser::parseStructInitializer(ASTNode* type_expr) {
    ...
    while (peek().type != TOKEN_RBRACE && !is_at_end()) {
        ...
        if (match(TOKEN_DOT)) {
            ...
            field_name_str = field_name.value.identifier;
            field_loc = field_name.location;
            ...
            plat_strcpy(buf, "_");
            char num_buf[21];
            plat_i64_to_string(index, num_buf, sizeof(num_buf));
...
ASTNode* Parser::parseImportStmt() {
    ...
    ASTNode* node = createNodeAt(NODE_IMPORT_STMT, import_token.location);
