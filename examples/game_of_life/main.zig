...
pub fn main() !void {
    ...
    // Set glider pattern
    // (1, 0), (2, 1), (0, 2), (1, 2), (2, 2)
    set(p_grid, 1, 0, Cell{ .Alive = {} });
    set(p_grid, 2, 1, Cell{ .Alive = {} });
    set(p_grid, 0, 2, Cell{ .Alive = {} });
    set(p_grid, 1, 2, Cell{ .Alive = {} });
    set(p_grid, 2, 2, Cell{ .Alive = {} });
    ...
    while (gen < 100) {
        ...
        while (y < HEIGHT) {
            var x: usize = 0;
            while (x < WIDTH) {
                const neighbors = countNeighbors(p_grid, x, y);
                const current = get(p_grid, x, y);
...
fn get(grid: *[800]Cell, x: usize, y: usize) Cell {
    if (x >= WIDTH or y >= HEIGHT) return Cell{ .Dead = {} };
    return grid.*[y * WIDTH + x];
}

fn set(grid: *[800]Cell, x: usize, y: usize, cell: Cell) void {
    if (x >= WIDTH or y >= HEIGHT) return;
    grid.*[y * WIDTH + x] = cell;
}

fn countNeighbors(grid: *[800]Cell, x: usize, y: usize) u8 {
    var count: u8 = 0;
    var dy: i32 = -1;
    while (dy <= 1) {
        var dx: i32 = -1;
