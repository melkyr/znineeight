...
pub fn main() void {
    ...
    while (true) {
        ...
        print_str("read_line len=");
        __bootstrap_print_int(len);
        print_str("\n");
        if (len < 0) break;
        if (len == 0) continue;
        ...
        print_str("tokenizer.input.len=");
        __bootstrap_print_int(@intCast(i32, tokenizer.input.len));
        print_str("\n");
        const expr = parser_mod.parse_expr(&tokenizer, &perm_sand, &temp_sand) catch |err| {
            var nl: [1]u8 = undefined;
            nl[0] = 10;
            print_str("Parse error: ");
            __bootstrap_print_int(@intCast(i32, @enumToInt(err)));
            print_str("\n");
            print_str(nl[0..1]);
            continue;
        };
