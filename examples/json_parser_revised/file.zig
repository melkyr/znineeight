...
pub fn readFile(arena: *void, path: []const u8) FileError![]u8 {
    ...
    const f = fopen(@ptrCast([*]const c_char, path.ptr), rb_str) orelse return error.OpenFailed;
    defer { _ = fclose(f); }

    if (fseek(f, 0, SEEK_END) != 0) return error.SeekFailed;
    const size_i64 = ftell(f);
    if (size_i64 < 0) return error.SeekFailed;
    if (fseek(f, 0, SEEK_SET) != 0) return error.SeekFailed;

    const size = @intCast(usize, size_i64);
    const buffer_void = arena_alloc_default(size);
    const buffer = @ptrCast([*]u8, buffer_void);
    const bytes_read = fread(buffer, 1, size, f);
    if (bytes_read != size) return error.ReadFailed;
