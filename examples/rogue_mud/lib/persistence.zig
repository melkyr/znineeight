...
// C89 File I/O
extern "c" fn fopen(path: [*]const u8, mode: [*]const u8) ?*u8;
extern "c" fn fwrite(ptr: *const u8, size: usize, count: usize, stream: *u8) usize;
extern "c" fn fread(ptr: *u8, size: usize, count: usize, stream: *u8) usize;
extern "c" fn fclose(stream: *u8) i32;
...
pub fn saveDungeon(arena: *sand_mod.Sand, dungeon: scenario.Dungeon_t, filename: []const u8) !void {
    ...
    if (fwrite(@ptrCast(*const u8, &header), 1, 2, file) != 2) return error.WriteFailed;

    const tile_count = @intCast(usize, dungeon.width) * @intCast(usize, dungeon.height);
    if (fwrite(@ptrCast(*const u8, dungeon.tiles.ptr), @sizeOf(tile_mod.Tile), tile_count, file) != tile_count) return error.WriteFailed;

    var counts = [2]usize{ @intCast(usize, dungeon.room_count), dungeon.entity_count };
    if (fwrite(@ptrCast(*const u8, &counts), @sizeOf(usize), 2, file) != 2) return error.WriteFailed;

    if (fwrite(@ptrCast(*const u8, dungeon.rooms.ptr), @sizeOf(room_mod.Room_t), @intCast(usize, dungeon.room_count), file) != @intCast(usize, dungeon.room_count)) return error.WriteFailed;
    if (fwrite(@ptrCast(*const u8, dungeon.entities.ptr), @sizeOf(entity_mod.Entity), dungeon.entity_count, file) != dungeon.entity_count) return error.WriteFailed;
}
...
pub fn loadDungeon(arena: *sand_mod.Sand, out: *scenario.Dungeon_t, filename: []const u8) !void {
    ...
    if (fread(@ptrCast(*u8, &header), 1, 2, file) != 2) return error.ReadFailed;
    ...
    if (fread(@ptrCast(*u8, tiles.ptr), @sizeOf(tile_mod.Tile), tile_count, file) != tile_count) return error.ReadFailed;

    var counts = [2]usize{ @intCast(usize, 0), @intCast(usize, 0) };
    if (fread(@ptrCast(*u8, &counts), @sizeOf(usize), 2, file) != 2) return error.ReadFailed;
    ...
    if (fread(@ptrCast(*u8, rooms.ptr), @sizeOf(room_mod.Room_t), room_count, file) != room_count) return error.ReadFailed;
    ...
    if (fread(@ptrCast(*u8, entities.ptr), @sizeOf(entity_mod.Entity), entity_count, file) != entity_count) return error.ReadFailed;
